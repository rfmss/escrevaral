...
- Legacy status: ES5 estrito — `acorn.parse` `ecmaVersion:5` PASS nas engines (sem lookbehind, sem `\p{L}`, sem flags `/u`, sem `normalize("NFD")` — `tokenizarFrases` por split `§`/lookahead, `normalizar` por mapa `DIACRITICS_PT_AN` + drop de combinantes); scanner de marcadores (só código): **0 marcadores modernos** — VERIFICADO em Node + **Chromium (Playwright)**
...
M4 justificado por 114/114 com ouro da fonte (fidelidade comportamento-for-comportamento, incluindo regressões de `options` booleano/número) + **revisão independente** (auditoria externa, achado A1 corrigido e coberto por banca; Astra R1–R6 aplicados e re-aprovados) + **QA de browser 51/51** (mesmas bancas rodando em Chromium via Playwright) + pureza ES5 9/9 + 43/43 adversarial sem crashes + cronometragem sem regressão. Não é M6 (falta medição do legado real em produção).
...
- 2026-09-12 — **avaliação do Astra (PR #165, `ASTRA-REVIEW-capsulas-m4.md`) → MERGE: NÃO, R1–R6** + correções aplicadas (resposta: `docs/ASTRA-REVIEW-resposta.md`). Neste domínio: R1 — lookbehind `(?<=[.!?…])` → split por `§` (lookahead ES5), `\p{L}`/`/u` → classes explícitas, `normalize("NFD")` → mapa + drop de combinantes, matchAll → polyfill ES5; R3 — severidade `alto→3/moderado→2/baixo→1`; R4 — `check()` lê `snapshot.context || snapshot.options` (contexto de poesia flui). Re-aprovado: analise 114/114, adversarial 43/43, es5-purity 9/9, integração 6/6.
