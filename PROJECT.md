...
| V8/Linux da oficina | Testes e medições locais | 450 verificações aprovadas, sintaxe ES5, equivalência portátil e cenários com rede/APIs modernas bloqueadas |
...
Editor local com título e hora fixa de criação, navegação mês → dia → notas, busca por título e texto, importação aditiva e exportação. Superfície gesso/grafite em claro/escuro, fonte local de escrita, foco opcional por parágrafo e linha ativa centralizada.
...
O arquivo portátil da entrega de engines tem 456.821 bytes. A bancada não representa tempo de WebKit, memória do iPad nem taxa geral de acerto linguístico. Medições e limites estão em `astra/MATURACAO.md`.
