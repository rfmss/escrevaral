...
(function analiseEngine(global) {
  ...
  const CONFUSOES_GRAMATICAIS = [
    ...
    [/\ba partir d[eoa]s?\b(?!\s+(?:então|desse|desse|daí|agora|hoje|amanhã|já|logo))/gi,
     "«a partir de» (introdução de argumento)",
     "Quando usada para introduzir argumentos ou análises, «a partir de» tende a ser vaga. Prefira «segundo», «com base em», «considerando»."],
    ...
    [/\bassim como\b/gi,
     "«assim como»",
     "Comparação que pode criar paralelismo falso. Verifique se os termos comparados são de fato equivalentes."],
    [/\btal qual\b/gi,
     "«tal qual»",
     "Comparação enfática. Certifique-se de que a equivalência afirmada é precisa e não apenas retórica."],
    [/\bda mesma forma (?:que|como)\b/gi,
     "«da mesma forma que»",
     "Paralelismo que pode esconder diferenças relevantes. Explicite em que aspecto são idênticos."],
    [/\bdo mesmo modo (?:que|como)\b/gi,
     "«do mesmo modo que»",
     "Construção comparativa que pode ser imprecisa. Especifique o modo compartilhado ou reformule."],
    [/\bao contrário d[eo]\b/gi,
     "«ao contrário de»",
     "Contraste válido, mas frequentemente usado sem precisar em que aspecto os termos se opõem."],
    [/\bem contrapartida\b/gi,
     "«em contrapartida»",
     "Conector de contraste formal. Certifique-se de que a oposição é real, não apenas aparente."],
    [/\bpor outro lado\b/gi,
     "«por outro lado»",
     "Conector de contraste muito frequente. Se o contraste é forte, prefira «mas», «porém», «contudo»."],
    [/\bapesar (?:de|disso)\b/gi,
     "«apesar de»",
     "Concessiva válida. Certifique-se de que a ressalva enriquece o argumento, não o enfraquece sem necessidade."],
    // batch 16
