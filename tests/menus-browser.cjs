...
(async()=>{
 ...
 const end=(await p.locator('#manuscrito').inputValue()).length;await select(p,end,end);assert.equal(await p.locator('#selection-tools').isVisible(),false);await p.click('#desk-paste');assert.equal(await p.locator('#manuscrito').inputValue(),'Maré Maré. O barco espera.Maré');
 ...
 for(const [width,height]of [[1366,654],[820,600],[390,844],[320,568],[667,375]]){
  ...
  for(const theme of ['claro','escuro']){
   ...
   await p.click('#selection-close');await p.waitForTimeout(60);assert.equal(await p.locator('#selection-tools').isVisible(),false,'seleção permanece recolhida');await fits(p,'#desk-paste');
   ...
   await select(p,0,7);await p.click('#mesa-toggle');await p.waitForTimeout(60);assert.equal(await p.locator('#selection-tools').isVisible(),false,'seleção recolhida em Ajustes');await fits(p,'#mesa');await fits(p,'#mesa-close');
 ...
 await p.setViewportSize({width:1366,height:654});await p.evaluate(()=>document.body.setAttribute('data-theme','claro'));await select(p,0,7);await p.click('#os-start');await p.click('#start-tools-toggle');assert.equal(await p.locator('#selection-tools').isVisible(),false,'seleção recolhida em Início');assert.equal(await p.locator('#start-tools-toggle').getAttribute('aria-expanded'),'true');await shot(p,'inicio');await p.keyboard.press('Escape');
