(function () {
    ...
    var fixture = JSON.parse(fs.readFileSync(
        path.resolve(__dirname, "fixtures/morphology-personal-infinitive-evaluation.json"),
        "utf8"
    ));
    ...
    function check(item) {
        ...
        if (!selected) {
            failures.push(item.id + ": consulta não encontrada");
            return;
        }
        ...
        if (!missing.length && !forbidden.length) {
            passed += 1;
            console.log("PASS " + item.id + " -> " + message);
        } else {
            failures.push(
                item.id + ": ausentes [" + missing.join(", ") +
                "]; proibidos [" + forbidden.join(", ") + "]; " + message
            );
        }
    }
