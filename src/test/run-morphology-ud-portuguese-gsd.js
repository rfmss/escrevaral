(function () {
    ...
    var fixture = JSON.parse(fs.readFileSync(
        path.resolve(__dirname, "fixtures/morphology-ud-portuguese-gsd.json"),
        "utf8"
    ));
    ...
    fixture.cases.forEach(function (item) {
        ...
        if (!item.shouldFind && !selected) {
            passed += 1;
            console.log("PASS " + item.id + " -> NOUN preservado");
            return;
        }
        if (!item.shouldFind && selected) {
            failures.push(item.id + ": falso positivo: " + selected.message);
            console.error("FAIL " + item.id + " -> " + selected.message);
            return;
        }
        if (item.shouldFind && !selected) {
            failures.push(item.id + ": VERB não encontrado");
            console.error("FAIL " + item.id + " -> VERB não encontrado");
            return;
        }

        var missing = item.includes.filter(function (value) {
            return selected.message.indexOf(value) < 0;
        });
        var forbidden = item.excludes.filter(function (value) {
            return selected.message.indexOf(value) >= 0;
        });
        if (!missing.length && !forbidden.length) {
            passed += 1;
            console.log("PASS " + item.id + " -> " + selected.message);
        } else {
            failures.push(
                item.id + ": ausentes [" + missing.join(", ") +
                "]; proibidos [" + forbidden.join(", ") + "]"
            );
        }
    });
