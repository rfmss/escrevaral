...
(function analiseEngine(global) {
  ...
  var SUBSTANTIVOS_VAGOS = new LSet([
  ...
  var PALAVRAS_FRACAS_ABERTURA = new LSet([
  ...
  function contarSilabas(palavra) {
    var v = normalizar(palavra).match(/[aeiouáéíóúâêôãõü]/gi);
    return v ? v.length : 1;
  }

  // ── STOPWORDS (para análise de repetição) ─────────────────────────────────

  var STOPWORDS = new LSet([
  ...
  function analisarConfusoes(texto) {
    var encontradas = [];
    CONFUSOES_GRAMATICAIS.forEach(function(m){ var padrao = m[0], rotulo = m[1], sugestao = m[2]; 
      var hits = toArray(texto.matchAll(padrao));
      if (hits.length > 0)
        encontradas.push({ rotulo: rotulo, sugestao: sugestao, ocorrencias: hits.length, exemplos: hits.slice(0, 2).map(function(m) { return (m[0]); }) });
    });
  ...
  function analisarPleonasmos(texto) {
    ...
    var seen = new LSet();
    var encontrados = PLEONASMOS
      .filter(function(m){ var p = m[0];  var k = p.toLowerCase(); return lower.includes(k) && !seen.has(k) && seen.add(k); })
      .map(function(m){ var p = m[0], corr = m[1];return (({
        rotulo: "«" + ("" + ((p))) + "»",
  ...
  function analisarEconomia(texto, frases, totalPalavras, contexto) {
    ...
    var palavras = tokenizarPalavras(texto);
    var lower = texto.toLowerCase();
    var skipPleonasmos = Boolean(contexto.skipPleonasmos);

    // 1. Adverbios -mente
    var adverbios = palavras.filter(function(p) { return (normalizar(p).endsWith("mente")); });
    var densidadeAdv = totalPalavras > 0 ? adverbios.length / totalPalavras : 0;

    // 2. Voz passiva aproximada
    var passiva = toArray(texto.matchAll(/(?:^|\s)(foi|foram|é|são|era|eram|será|serão|seria|seriam|fosse|fossem|tenha sido|tenham sido|tem sido|têm sido|está sendo|estava sendo|estão sendo)\s+\w+(?:ado|ada|ados|adas|ido|ida|idos|idas)\b/gim));
    var propPassiva = frases.length > 0 ? passiva.length / frases.length : 0;

    // 3. Redundância / pleonasmos
    var redEncontradas = skipPleonasmos
      ? []
      : PLEONASMOS.filter(function(m){ var p = m[0];return (lower.includes(p.toLowerCase())); }).map(function(m){ var p = m[0];return (p); });

    // 4. Negação dupla/indireta
    var negacoes = [];
    NEGACOES_DUPLAS.forEach(function(re) {
      var matches = toArray(texto.matchAll(re));
      Array.prototype.push.apply(negacoes, matches.map(function (m) { return m[0]; }));
    });

    return {
      adverbiosMente: { valor: adverbios.length, densidade: +(densidadeAdv * 100).toFixed(1), lista: adverbios.slice(0, 8) },
      vozPassiva: { ocorrencias: passiva.length, proporcao: +(propPassiva * 100).toFixed(1), exemplos: passiva.slice(0, 3).map(function(m) { return (m[0]); }) },
      redundancia: { ocorrencias: redEncontradas.length, lista: redEncontradas },
      negacaoDupla: { ocorrencias: negacoes.length, lista: toArray(new LSet(negacoes)).slice(0, 5) },
    };
  }
  ...
  function analisarClareza(frases, totalPalavras) {
    var comprimentos = frases.map(function(f) { return (tokenizarPalavras(f).length); }).filter(function(n) { return (n > 0); });
    if (!comprimentos.length) return {};

    var media = comprimentos.reduce(function(a, b) { return (a + b); }, 0) / comprimentos.length;
    var dp = Math.sqrt(comprimentos.reduce(function(a, b) { return (a + Math.pow(b - media, 2)); }, 0) / comprimentos.length);

    // Pronomes ambíguos — alta densidade de pronome de 3ª pessoa após múltiplos substantivos
    var frasesComEleEla = frases.filter(function(f) {
      var ws = tokenizarPalavras(f).map(normalizar);
      var ele = ws.filter(function(w) { return (["ele","ela","eles","elas","o","a","os","as","lhe","lhes","lo","la"].includes(w)); }).length;
      var subs = ws.filter(function(w) { return (w.length > 3 && !STOPWORDS.has(w)); }).length;
      return ele >= 2 && subs >= 2;
    });

    // Subordinação: conta conectivos subordinativos por frase
    var SUBORD = ["que","quando","enquanto","se","porque","pois","embora","ainda que","mesmo que","apesar","caso","para que","a fim","salvo","exceto","conforme","segundo","como"];
    var frasesComSubord = frases.filter(function(f) {
      var lower = f.toLowerCase();
      var count = SUBORD.filter(function(s) { return (lower.includes(" " + ("" + ((s))) + " ") || lower.startsWith(("" + ((s))) + " ")); }).length;
      return count >= 3;
    });

    // Coerência de tempo verbal (detecção de mistura presente/passado)
    var PRES = /\b(é|são|está|estão|tem|têm|faz|fazem|vai|vão|pode|podem|deve|devem|fica|ficam)\b/gi;
    var PASS = /\b(foi|foram|era|eram|estava|estavam|tinha|tinham|fez|fizeram|foi|foram|pôde|puderam|devia|deviam|ficou|ficaram)\b/gi;
    var mistura = 0;
    frases.forEach(function(f) {
      var hasPres = PRES.test(f);
      var hasPass = PASS.test(f);
    ...
    return {
      ...
      frasesLongas: comprimentos.filter(function(n) { return (n > 35); }).length,
      frasesVeryLong: comprimentos.filter(function(n) { return (n > 50); }).length,
  ...
  function analisarRitmo(texto, frases) {
    ...
    var curtas = comprimentos.filter(function(n) { return (n < 8); }).length;
    var medias = comprimentos.filter(function(n) { return (n >= 8 && n <= 20); }).length;
    var longas = comprimentos.filter(function(n) { return (n > 20); }).length;
    var total = comprimentos.length;

    // Repetição lexical próxima (janela de 3 frases)
    var lemmas = frases.map(function(f) { return (tokenizarPalavras(f).map(normalizar).filter(function(w) { return (w.length > 3 && !STOPWORDS.has(w)); })); }
    );
    var repeticoes = [];
    for (var i = 0; i < lemmas.length; i++) {
      var janela = new LSet(toArray(lemmas[i-1]||[]).concat(toArray(lemmas[i+1]||[]), toArray(lemmas[i+2]||[])));
      lemmas[i].forEach(function(w) {
    ...
    var aberturasFracas = 0;
    paragrafos.forEach(function(p) {
      var primeiraPalavra = normalizar((tokenizarPalavras(p.trim())[0]) || "");
  ...
  function analisarVoz(texto, contexto) {
    if (contexto === undefined) contexto = {};
    var lower = texto.toLowerCase();
    var encontrados = (contexto.skipCliches ? [] : CLIQUES_PT.filter(function(c) { return (lower.includes(c)); }));
    return {
  ...
  function analisarEstrutura(texto, frases, totalPalavras) {
    ...
    if (paragrafos.length >= 3) {
      var pWords = paragrafos.map(function(p) { return (contarPalavras(p)); });
      var total = pWords.reduce(function(a, b) { return (a + b); }, 0);
      var intro = pWords[0];
      var conclusao = pWords[pWords.length - 1];
      proporcao = {
    ...
    var comTransicao = 0;
    paragrafos.forEach(function(p) {
      var primeiraFrase = p.trim().toLowerCase().slice(0, 80);
      if (CONECTIVOS_LOGICOS.some(function(c) { return (primeiraFrase.startsWith(c) || primeiraFrase.includes(" " + ("" + ((c))) + " ")); })) {
        comTransicao++;
      }
    });
  ...
  function analisarPov(frases) {
    var EU = /\b(eu|me|mim|meu|minha|meus|minhas)\b/gi;
    var NOS = /\b(n[oó]s|nosso|nossa|nossos|nossas|a gente)\b/gi;
    var ELE = /\b(ele|ela|eles|elas|lhe|lhes|seu|sua|seus|suas)\b/gi;
    var AUTOR = /\b(o autor|a autora|o escritor|a escritora|o narrador|a narradora)\b/gi;

    var frasesEu = 0, frasesNos = 0, frasesEle = 0, frasesAutor = 0;
    frases.forEach(function(f) {
    ...
    var total = frases.length || 1;
    var prop1a  = frasesEu / total;
    var prop3a  = frasesEle / total;
    var mistura = Math.min(prop1a, prop3a);
    var ambosSignificativos = prop1a > 0.15 && prop3a > 0.15;
  ...
  function analisarLexico(texto, totalPalavras) {
    var palavras = tokenizarPalavras(texto);
    var normas = palavras.map(normalizar);
    var lower = palavras.map(function(w) { return (w.toLowerCase()); });

    var estado = lower.filter(function(w) { return (VERBOS_ESTADO.has(w)); });
    var vagos = normas.filter(function(w) { return (SUBSTANTIVOS_VAGOS.has(w)); });
    var totalVerbs = normas.filter(function(w) { return (/^.+(ar|er|ir|ou|ei|ava|ia|ará|erá|sse|endo|ando|indo|ado|ada|ido|ida)$/.test(w)); }
    ).length;
    ...
    return {
      ...
      substantivosVagos: {
        ...
        lista: toArray(new LSet(vagos)).slice(0, 8),
      },
    };
  }
  ...
  function inferirContextoAnalise(options) {
    ...
    var formato = ("" + ((options.formato || options.editorMode || options.oficio || options.kind || options.type || ""))).toLowerCase();
    var poesia = options.poesia === true || /poema|poesia|soneto|slam|haiku|cordel|verso/.test(formato);
  ...
  function analisar(texto, options) {
    ...
    var contexto = inferirContextoAnalise(options);
    var frases = tokenizarFrases(texto.trim());
    var totalPalavras = contarPalavras(texto);
    var totalFrases = frases.length;
    var totalParagrafos = texto.split(/\n\s*\n/).filter(function(p) { return (p.trim().length > 10); }).length;

    // Legibilidade Flesch-BR
    var allWords = tokenizarPalavras(texto);
    var totalSilabas = allWords.reduce(function(a, w) { return (a + contarSilabas(w)); }, 0);
    var totalSentencas = Math.max(1, texto.split(/[.!?]+/).filter(function(s) { return (s.trim()); }).length);
    var flesch = Math.max(0, Math.min(100, Math.round(
  ...
  function interpretarResultado(resultado) {
    if (!resultado) return [];
    var alertas = [];
    var economia = resultado.economia, clareza = resultado.clareza, ritmo = resultado.ritmo, voz = resultado.voz, lexico = resultado.lexico, pov = resultado.pov, norma = resultado.norma;
    ...
    if ((resultado.pleonasmos ? resultado.pleonasmos.ocorrencias : 0) > 0)
      resultado.pleonasmos.lista.forEach(function(p) {
    ...
    if ((resultado.confusoes ? resultado.confusoes.ocorrencias : 0) > 0)
      resultado.confusoes.lista.forEach(function(c) {
    ...
    if ((norma && norma.pontuacao && norma.pontuacao.issues ? norma.pontuacao.issues.length : 0) > 0) {
      var first = norma.pontuacao.issues[0];
      var contextualEvidence = first.contextual && (first.evidence ? first.evidence.length : 0)
    ...
    if ((clareza.tempoVerbal ? clareza.tempoVerbal.frasesComMistura : 0) > 2 && resultado.meta.totalFrases > 10)
    ...
    if ((clareza.pronomeAmbiguo ? clareza.pronomeAmbiguo.suspeitas : 0) > 3)
    ...
    if ((ritmo.aberturaFracos ? ritmo.aberturaFracos.aberturasFracas : 0) >= 3 && resultado.meta.totalParagrafos >= 3)
    ...
    return alertas.sort(function(a, b) { return ((a.nivel === "alto" ? -1 : 1) - (b.nivel === "alto" ? -1 : 1)); });
  }
  ...
  global.VeredaAnalise = {
    analisar: analisar,
    analisarConfusoes: analisarConfusoes,
    interpretarResultado: interpretarResultado,
    tokenizarFrases: tokenizarFrases,
    tokenizarPalavras: tokenizarPalavras,
...
(function (global) {
    ...
    /* Escrevaral-Encore — engine de Análise Literária / Vereda v3 (port ES5).
     * Reuso de comportamento (fonte A, escrevaral/analise-engine.js), não cópia.
     * Contrato check(snapshot, done): devolve Findings por callback (assíncrono).
     */

    function AnaliseLiterariaEngine() {
        this.id = "ANALISE-LITERARIA";
        this.domain = "analise-literaria";
        this.version = "3.0.0";
    }

    AnaliseLiterariaEngine.prototype.analisar = function (texto, options) {
        if (!global.VeredaAnalise) return null;
        return global.VeredaAnalise.analisar(texto, options);
    };

    AnaliseLiterariaEngine.prototype.interpretarResultado = function (resultado) {
        if (!global.VeredaAnalise) return [];
        return global.VeredaAnalise.interpretarResultado(resultado);
    };
    ...
    AnaliseLiterariaEngine.prototype.check = function (snapshot, done) {
        ...
        setTimeout(function () {
            var findings = [];
            var contracts = global.Encore && global.Encore.contracts;
            try {
                var r = self.analisar(text, options);
                ...
                if (contracts) {
                    findings.push(new contracts.Finding(self.id, [0, Math.max(1, text.length)],
                        "Erro interno: " + (err && err.message ? err.message : err), 1, 1));
                }
            }
            done(findings);
        }, 0);
    ...
    if (typeof module !== "undefined" && module.exports) {
        module.exports = AnaliseLiterariaEngine;
    } else {
        global.Encore.core = global.Encore.core || {};
        global.Encore.core.engines = global.Encore.core.engines || {};
        global.Encore.core.engines.AnaliseLiterariaEngine = AnaliseLiterariaEngine;
    }
