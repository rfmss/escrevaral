/* Vereda v3 — port ES5 puro (conversão mecânica auditada + validação por
 * fidelidade comportamental contra a fonte ES6 em bancas congeladas). */
if (!String.prototype.includes) {
  String.prototype.includes = function (s) { return this.indexOf(String(s)) !== -1; };
}
if (!String.prototype.endsWith) {
  String.prototype.endsWith = function (s) { s = String(s); return this.length >= s.length && this.slice(this.length - s.length) === s; };
}
if (!String.prototype.startsWith) {
  String.prototype.startsWith = function (s) { s = String(s); return this.slice(0, s.length) === s; };
}
if (!String.prototype.matchAll) {
  String.prototype.matchAll = function (re) {
    ...
    while ((m = re.exec(str)) !== null) {
      arr = [];
      for (i = 0; i < m.length; i++) arr[i] = m[i];
      arr.index = m.index;
      arr.input = m.input;
      arr.groups = m.groups || undefined;
      list.push(arr);
      if (m[0] === '') re.lastIndex++;
    }
    return list;
...
function LSet(initial) {
  this.map = Object.create ? Object.create(null) : {};
  this.list = [];
  var i, cur, j;
  if (initial) for (i = 0; i < initial.length; i++) {
    cur = initial[i];
    if (Array.isArray(cur)) { for (j = 0; j < cur.length; j++) this.add(cur[j]); }
    else this.add(cur);
  }
}
LSet.prototype.add = function (k) { if (!hasOwn(this.map, k)) { this.map[k] = true; this.list.push(k); } return this; };
LSet.prototype.has = function (k) { return hasOwn(this.map, k); };
LSet.prototype.values = function () { return this.list.slice(); };
if (!Array.prototype.includes) {
  Array.prototype.includes = function (v) { for (var i = 0; i < this.length; i++) if (this[i] === v) return true; return false; };
}
function LMap(pairs) {
  this.m = Object.create ? Object.create(null) : {};
  var i;
  for (i = 0; i < pairs.length; i++) this.m[String(pairs[i][0])] = [pairs[i][1]];
}
LMap.prototype.has = function (k) { return hasOwn(this.m, String(k)); };
LMap.prototype.get = function (k) {
  var v = this.m[String(k)];
  return v ? v[0] : undefined;
};
function toArray(it) {
  if (Array.isArray(it)) return it.slice();
  if (it && typeof it.values === 'function') return it.values();
  var out = [], step, r, i;
  if (it && typeof Symbol !== 'undefined' && it[Symbol.iterator]) {
    step = it[Symbol.iterator]();
    while (!(r = step.next()).done) out.push(r.value);
    return out;
  }
  for (i = 0; i < it.length; i++) out.push(it[i]);
  return out;
}
(function lexicalEngine(global) {
  var localLexicon = {};
  var functionWords = { artigos:[], contracoes:[], conjuncoes:[], preposicoes:[], pronomes:[], adverbios:[] };
  var _loaded = false;
  var _loadError = false;
  var _VERBOS_PRES_SET = null;  // Set de formas do presente (3ª p.) — carregado de norma-data.json
  var _FORMAS_IRR_SET  = null;  // Set de formas verbais irregulares

  function ensureLoaded() {
    if (_loaded || _loadError) return;
    try {
      var _g = typeof global !== 'undefined' ? global : window;
      var _d = _g.Encore && _g.Encore.data;
      if (!_d || !_d.lexicalData || !_d.lexicalNormaData) { _loadError = true; return; }
      localLexicon  = _d.lexicalData.localLexicon  || {};
      functionWords = _d.lexicalData.functionWords || functionWords;
      if (_d.lexicalNormaData.verbos_pres_reg)  _VERBOS_PRES_SET = new LSet(_d.lexicalNormaData.verbos_pres_reg);
      if (_d.lexicalNormaData.formas_verbais_irr) _FORMAS_IRR_SET = new LSet(_d.lexicalNormaData.formas_verbais_irr);
      _loaded = true;
    } catch (_) {
      _loadError = true;
    }
  ...
  // Carregar na inicialização (dados embutidos, síncrono)
  ensureLoaded();
  ...
  var PRON_DEMONSTRATIVOS = new LSet([
  ...
  var POLISSEMIA = {
    ...
    "como": function(prev, next) {
      if (!prev) return "Advérbio";
      var _p = normalizeWord(prev);
    ...
    "cujo":  function() { return ("Pronome relativo"); },
    "cuja":  function() { return ("Pronome relativo"); },
    "cujos": function() { return ("Pronome relativo"); },
    "cujas": function() { return ("Pronome relativo"); },
    ...
    "mas": function() { return ("Conjunção"); },
    "nem": function() { return ("Conjunção"); },
    "porem": function() { return ("Conjunção"); },
    "contudo": function() { return ("Conjunção"); },
    "todavia": function() { return ("Conjunção"); },
    "entretanto": function() { return ("Conjunção"); },
    "logo": function(prev) {
    ...
    "bem": function(prev, next) {
      ...
      var ARTIGOS_PREP = new LSet(["o","a","um","uma","do","da","no","na","ao","á","pelo","pela","todo","toda","pelo","por"]);
    ...
    "mesmo": function(prev, next) {
      ...
      var DET = new LSet(["o","a","os","as","um","uma","do","da","no","na","ao"]);
      if (prev && DET.has(prev)) return "Adjetivo";
      // "mesmo assim" / "mesmo que" / início → Advérbio concessivo
      return "Advérbio";
    },

    "so": function(prev, next) {
      // "estava só" / "ficou só" / "vive só" → após cópula → Adjetivo predicativo
      var COPULAS = new LSet(["estava","estou","fiquei","ficou","ficara","era","senti","sente","vive","anda","parecia","permanecia","ficasse"]);
    ...
    "caso": function(prev, next) {
      // Após artigo/determinante/contração → Substantivo
      var ART = new LSet(["o","a","os","as","um","uma","do","da","dos","das","no","na","nos","nas","num","numa","ao","aos","este","esse","aquele","neste","nesse","naquele","nesta","nessa","naquela","tal","cada","meu","seu","nosso","teu","qualquer"]);
      if (prev && ART.has(prev)) return "Substantivo";
      // Default → Conjunção condicional
      return "Conjunção";
    },

    "visto": function(prev, next) {
      // Após auxiliar de voz passiva / perfeito composto → Verbo (particípio)
      var AUX = new LSet(["havia","tinha","teria","tenho","tem","temos","tinham","foram","foi","sendo","tendo","haviamos","tinhamos","fosse","sera","sido","ser","ter"]);
      if (prev && AUX.has(prev)) return "Verbo (particípio)";
      // Após cópula predicativa → Adjetivo ("estava visto", "ficou visto")
      var COPULAS_V = new LSet(["estava","ficou","fica","esta","e","era","eram","pareceu","parecia","continua"]);
      if (prev && COPULAS_V.has(prev)) return "Adjetivo";
      // Após artigo/contração/determinante → Substantivo (documento de viagem)
      var ART = new LSet(["o","a","os","as","um","uma","do","da","dos","das","no","na","nos","nas","ao","meu","seu","nosso","teu","este","esse","aquele","neste","nesse","naquele"]);
      if (prev && ART.has(prev)) return "Substantivo";
      return "Verbo (particípio)";
    },

    "posto": function(prev, next) {
      // "posto que" → Conjunção causal/concessiva
      if (next && normalizeWord(next) === "que") return "Conjunção";
      // Após auxiliar de voz passiva → Verbo (particípio)
      var AUX = new LSet(["foi","sera","fora","fosse","sendo","sido","foram","ser","ter","tendo"]);
      if (prev && AUX.has(prev)) return "Verbo (particípio)";
      // Após cópula predicativa → Adjetivo ("ficou posto", "estava posto", "é posto")
      var COPULAS_P = new LSet(["ficou","fica","ficava","estava","esta","e","era","eram","pareceu","parecia"]);
      if (prev && COPULAS_P.has(prev)) return "Adjetivo";
      // Após artigo/contração/determinante → Substantivo (local físico ou cargo)
      var ART = new LSet(["o","a","os","as","um","uma","do","da","dos","das","no","na","nos","nas","num","numa","ao","aos","meu","seu","nosso","teu","este","esse","aquele","neste","nesse","naquele","nesta","nessa","naquela","cada","qualquer"]);
      if (prev && ART.has(prev)) return "Substantivo";
      return "Verbo (particípio)";
    },

    "tanto": function(prev, next) {
    ...
    "ora": function(prev, next) {
      ...
      var PART_INTERJ = new LSet(["isso","este","esse","aquilo","voce","eles","elas","nos","que","bom"]);
      if (next && PART_INTERJ.has(normalizeWord(next))) return "Interjeição";
      // Início + próxima é modal negativo → Interjeição ("Ora, não!")
      var PART_NEG_MODAL = new LSet(["nao","nem","ja","bem","la","ve","olha","eis","basta","veja","xii"]);
      if (!prev && next && PART_NEG_MODAL.has(normalizeWord(next))) return "Interjeição";
      return "Advérbio";
    },

    "morto": function(prev, next) {
      // Após artigo → Substantivo nominalizado ("um morto", "o morto")
      var ART = new LSet(["um","uma","o","a","os","as","do","da","dos","das","no","na","nos","nas","ao","aos"]);
      if (prev && ART.has(prev)) return "Substantivo";
      // Após cópula → Adjetivo predicativo ("estava morto", "caiu morto")
      var COPULAS = new LSet(["estava","estou","ficou","ficara","era","parecia","caiu","caira","cai"]);
      if (prev && COPULAS.has(prev)) return "Adjetivo";
      return "Verbo (particípio)";
    },
  };
  ...
  (function() {
    ...
    POLISSEMIA["quem"] = function(prev) {
      ...
      var PREP_ART = new LSet(["a","de","em","para","por","com","sobre","ante","entre","sem",
    ...
    var _COMP_QUANTO = new LSet(["tanto","tanta","tantos","tantas","mais","menos","tao",
    ...
    POLISSEMIA["nos"] = function(prev, next) {
      // "nos" antes de substantivo/adjetivo (sem morfologia verbal) → contração "em + os"
      if (next && next.length > 2) {
        var _verbSfx = /(ou|eu|iu|avam|iam|ava|ia|ar|er|ir|ando|endo|indo|aram|eram|iram)$/.test(next);
    ...
    POLISSEMIA["presente"] = function(prev) {
      var ART = new LSet(["o","a","os","as","um","uma","do","da","dos","das","no","na","ao","num","numa"]);
      if (prev && ART.has(prev)) return "Substantivo";
      return "Adjetivo"; // "estava presente", "ficou presente", "dia presente"
    };

    // "seguinte/seguintes" — quase sempre adjetivo em norma padrão
    POLISSEMIA["seguinte"]  = function() { return ("Adjetivo"); };
    POLISSEMIA["seguintes"] = function() { return ("Adjetivo"); };
    ...
    POLISSEMIA["tais"] = function() { return ("Pronome indefinido"); };
    POLISSEMIA["tal"]  = function(prev, next) {
    ...
    POLISSEMIA["vez"] = function(prev, next) {
    ...
    var _ARTS_COMP = new LSet(["o","a","os","as","um","uma","do","da","dos","das","no","na"]);
    POLISSEMIA["melhor"] = function(prev) { return (prev && _ARTS_COMP.has(prev) ? "Adjetivo" : "Advérbio"); };
    POLISSEMIA["pior"]   = function(prev) { return (prev && _ARTS_COMP.has(prev) ? "Adjetivo" : "Advérbio"); };
    POLISSEMIA["melhores"] = function(prev) { return (prev && _ARTS_COMP.has(prev) ? "Adjetivo" : "Advérbio"); };
    POLISSEMIA["piores"]   = function(prev) { return (prev && _ARTS_COMP.has(prev) ? "Adjetivo" : "Advérbio"); };
    ...
    POLISSEMIA["junto"] = function() { return ("Advérbio"); };
    POLISSEMIA["juntos"] = function() { return ("Advérbio"); };
    POLISSEMIA["juntas"] = function() { return ("Advérbio"); };
    ...
    var _ARTS_TARDE = new LSet(["o","a","os","as","um","uma","uns","umas","do","da","dos","das","no","na","pelo","pela"]);
    POLISSEMIA["tarde"] = function(prev) {
    ...
    POLISSEMIA["quando"] = function(prev, next) {
    ...
    POLISSEMIA["mal"] = function(prev, next) {
      ...
      var _ART = new LSet(["o","a","os","as","um","uma","do","da","dos","das","no","na"]);
    ...
    POLISSEMIA["pois"] = function(prev, next) {
    ...
    POLISSEMIA["segundo"] = function(prev, next) {
      var _ARTS = new LSet(["o","a","os","as","um","uma","do","da","no","na","ao","em"]);
    ...
    POLISSEMIA["nosso"] = function(prev, next) { return ("Pronome possessivo"); }; // "nosso lar" — sempre possessivo (masc.)

    // ── Grupo A — Adjetivos que funcionam como advérbios de modo ──
    // Bechara §advérbio: adjetivo após verbo de ação = advérbio de modo
    (function() {
      var _verbPast = /^.{2,}(ou|eu|iu|ava|ia|ando|endo|indo|aram|eram|iram)$/;
      function _adjAdv(prev, next) {
        if (prev && _verbPast.test(prev)) return "Advérbio"; // "voou alto", "custou caro"
        return "Adjetivo";
      }
      var __iter_0 = toArray(["alto","baixo","rapido","lento","duro","forte","barato","errado"]);
for (var __i_0 = 0; __i_0 < __iter_0.length; __i_0++) {
  var w = __iter_0[__i_0]; 
      ...
      var _COPULAS_CLARO = new LSet(["ficou","ficava","ficara","ficavam","fica","fique","ficasse","estava","esteve","estou","estamos","estavam","esta","e","era","eram","foi","fora","fosse","parecia","parece","pareceu","continua","continuava","permanecia"]);
      POLISSEMIA["claro"] = function(prev, next) {
      ...
      POLISSEMIA["fundo"] = function(prev) {
        var _PREPS_FUNDO = new LSet(["no","ao","do","pelo","pelo","um","uma","o","a","os","as","seu","sua","desse","deste","nesse","neste","seu","sua","outro","outra","todo","qualquer"]);
        if (!prev || _PREPS_FUNDO.has(prev)) return "Substantivo";
        return "Adjetivo"; // "voz funda" → Adjetivo
      };
      POLISSEMIA["funda"] = function() { return ("Adjetivo"); };
  ...
  (function() {
    ...
    var _ADJ_SFX = /(oso|osa|avel|ivel|ente|ante|ivo|iva|ico|ica|ino|ina|ado|ido|al|el|il|oz|iz)$/;
    // Adjetivos irregulares (sem sufixo canônico) que "muito" intensifica
    var _ADJ_IRR = new LSet([
    ...
    var _ADV_C   = new LSet(["antes","depois","cedo","tarde","longe","perto","bem","mal","mais","menos",
    ...
    var _GRAM_N  = new LSet(["e","ou","mas","que","se","quando","como","porque","pois","nem","para","por"]);
    ...
    POLISSEMIA["muita"]  = function() { return ("Pronome indefinido"); };
    // Plurais — uso substantivado ou adjetivo antes de noun (Pronome indefinido)
    POLISSEMIA["muitos"] = function(prev, next) {
      // "muitos chegaram" → Pronome indefinido (sujeito antes de verbo)
      if (next && next.length > 3 && /(ou|eu|iu|avam|iam|ava|ia|am|em)$/.test(next))
        return "Pronome indefinido";
      return "Pronome indefinido";
    };
    POLISSEMIA["muitas"] = function() { return ("Pronome indefinido"); };

    // "pouco" (sing. masc.) — mesma lógica de "muito"
    POLISSEMIA["pouco"]  = _intens;
    // "pouca" (sing. fem.) — antes de substantivo fem.
    POLISSEMIA["pouca"]  = function() { return ("Pronome indefinido"); };
    // Plurais de pouco
    POLISSEMIA["poucos"] = function() { return ("Pronome indefinido"); };
    POLISSEMIA["poucas"] = function() { return ("Pronome indefinido"); };
    ...
    POLISSEMIA["la"] = function() { return ("Advérbio"); };
    ...
    POLISSEMIA["ai"] = function(prev, next) {
    ...
    POLISSEMIA["alem"] = function(prev, next) {
      // "além de" → Preposição
    ...
    POLISSEMIA["ja"] = function(prev, next) {
    ...
    POLISSEMIA["assim"] = function(prev, next) {
    ...
    POLISSEMIA["conforme"] = function(prev, next) {
      if (next === "que") return "Conjunção";
      return "Preposição";
    };
    ...
    var _ARTS_DEF = new LSet(["o","a","os","as","um","uma","uns","umas"]);
    var _gentilicoPol = function(prev) { return (prev && _ARTS_DEF.has(prev) ? "Substantivo" : "Adjetivo"); };
    var _GENTILICOS = [
    ...
    var __iter_1 = toArray(_GENTILICOS);
for (var __i_1 = 0; __i_1 < __iter_1.length; __i_1++) {
  var g = __iter_1[__i_1]; 
  ...
  (function() {
    var _SUJPRON = new LSet(["eu","tu","ele","ela","nos","eles","elas","voce","voces","isso","aquilo","a gente","me","te","se","lhe"]);
    var _COPULAS_ADJ = new LSet(["ficou","ficava","ficara","ficavam","fica","fique","ficasse","estava","esteve","estou","estamos","estavam","esta","e","era","eram","foi","fora","fosse","parecia","parece","pareceu","continua","continuava","permanecia","tornava","tornou","se tornou"]);
    var _ARTS_ADJ = new LSet(["o","a","os","as","um","uma","uns","umas","do","da","dos","das","ao","aos"]);
    ...
    POLISSEMIA["tunica"] = function() { return ("Substantivo"); };
    POLISSEMIA["tunicas"] = function() { return ("Substantivo"); };
  })();
  ...
  var VERBOS_ACENTUADOS = new LMap([
  ...
  function analyze(word, text) {
    if (!((word ? word.trim() : undefined))) return null;
    var selectedWord = word.trim();
    // P0.3: acento distintivo vence normalização — checar antes de strip
    if (VERBOS_ACENTUADOS.has(selectedWord)) {
      var fixedClass = VERBOS_ACENTUADOS.get(selectedWord);
      var normalized = normalizeWord(selectedWord);
      var lexiconEntry = localLexicon[normalized];
      return {
        word: normalized,
        displayWord: ((lexiconEntry ? lexiconEntry.label : undefined) || selectedWord),
        className: ((lexiconEntry ? lexiconEntry.className : undefined) || fixedClass),
        functionName: inferFunctionName(fixedClass),
        field: ((lexiconEntry ? lexiconEntry.field : undefined) || inferSemanticField(normalized, fixedClass)),
        note: ((lexiconEntry ? lexiconEntry.note : undefined) || createLocalNote(fixedClass, normalized)),
        definicao: ((lexiconEntry ? lexiconEntry.definicao : undefined) || inferDefinicao(normalized, fixedClass)),
    ...
    var _CLITICOS = new LSet(["me","te","se","o","a","lo","la","lhe","nos","vos","lhes","los","las","mo","to","lho","no","vo"]);
    var _parts = selectedWord.split("-");
    var _isCliticizado = _parts.length >= 2 && _parts.slice(1).every(function(p) { return (_CLITICOS.has(p.toLowerCase()) || /^(ei|as|a|emos|eis|ao|aria|arias|ariam|ariamos)$/.test(p.toLowerCase())); });
    var _baseVerb = _isCliticizado ? _parts[0] : null;
    var lexiconEntry = localLexicon[normalized];
    ...
    var _polisOverride = text && (POLISSEMIA[normalized] || ((lexiconEntry && lexiconEntry.className ? lexiconEntry.className.startsWith("Verbo (particípio)") : undefined)));
    var className = (!_polisOverride && (lexiconEntry ? lexiconEntry.className : undefined))
    ...
    var alternatives = (ALTERNATIVAS[normalized] || []).filter(function(alt) {
      var altClass = alt.split(" — ")[0];
    ...
    return {
      ...
      definicao: ((lexiconEntry ? lexiconEntry.definicao : undefined) || inferDefinicao(normalized, className)),
  ...
  function inferWordClass(normalized, original) {
    ...
    var _PERF_IRR_ISSE = new LSet(["disse","disseste","dissemos","dissestes","disseram"]);
  ...
  var GATILHOS_SUBJ_PRES = new LSet([
  ...
  function inferWordClassContextual(word, text, prevNorm, nextNorm) {
    ...
    if (text && (prevNorm === undefined || nextNorm === undefined)) {
      var _tks = tokenizeWords(text);
      var _idx = _tks.findIndex(function(t) { return (normalizeWord(t) === normalized); });
      if (_idx !== -1) {
    ...
    if (text && LOCUCOES[normalized]) {
      var textNorm = normalizeWord(text).toLowerCase();
      var wordPos  = textNorm.indexOf(normalized);
      if (wordPos !== -1) {
        var __iter_2 = toArray(LOCUCOES[normalized]);
for (var __i_2 = 0; __i_2 < __iter_2.length; __i_2++) {
  var __o = __iter_2[__i_2]; var loc = __o.loc, classe = __o.classe; 
          var locNorm = loc.split(" ").map(normalizeWord).join(" ");
    ...
    if (POLISSEMIA[normalized]) {
      var result = POLISSEMIA[normalized](prevNorm, nextNorm, prev2Norm);
    ...
    if (prevNorm) {
      ...
      if (GATILHOS_SUBJ_PRES.has(prevNorm)) {
        ...
        if (/^.{2,}(e|es|em|emos|eis)$/.test(normalized)
            && !((functionWords.artigos ? functionWords.artigos.includes(normalized) : undefined))
            && !((functionWords.preposicoes ? functionWords.preposicoes.includes(normalized) : undefined)))
    ...
    var _rawClass = inferWordClass(normalized, original);
  ...
  var SEM_MOV  = new LSet(["andar","caminhar","correr","voar","saltar","deslizar","arrastar","subir","descer","entrar","sair","chegar","partir","vir","ir","voltar","avançar","recuar","fugir","perseguir","escalar","mergulhar"]);
  var SEM_DIC  = new LSet(["dizer","falar","gritar","sussurrar","murmurar","responder","perguntar","declarar","afirmar","negar","confessar","contar","narrar","relatar","anunciar","prometer","jurar","chamar","nomear"]);
  var SEM_COG  = new LSet(["pensar","saber","lembrar","esquecer","imaginar","sonhar","acreditar","duvidar","reconhecer","perceber","notar","descobrir","compreender","intuir","raciocinar","calcular","decidir","planejar"]);
  var SEM_EMO  = new LSet(["amar","odiar","temer","alegrar","entristecer","angustiar","desesperar","chorar","rir","sorrir","suspirar","tremer","estremecer","emocionar","apaixonar","magoar","consolar","agitar"]);
  var SEM_PER  = new LSet(["ver","olhar","enxergar","ouvir","escutar","sentir","tocar","cheirar","provar","observar","contemplar","fitar","espreitar","farejar","apalpar","tatear"]);

  var SUBST_TEMPO    = new LSet(["tempo","hora","momento","instante","segundo","minuto","dia","noite","manhã","tarde","semana","mes","ano","seculo","epoca","era","eterno","efemero","duraçao","ritmo","prazo","futuro","passado","presente"]);
  var SUBST_ESPACO   = new LSet(["lugar","espaço","casa","quarto","sala","rua","cidade","campo","floresta","mar","rio","caminho","estrada","trilha","praia","montanha","vale","deserto","ilha","continente","horizonte","limite","fronteira"]);
  var SUBST_LUZ      = new LSet(["luz","sombra","escuridao","claridade","brilho","reflexo","lampejo","trevas","penumbra","aurora","crepusculo","sol","lua","estrela","fogo","chama","brasa","faísca","neon","vela"]);
  var SUBST_CORPO    = new LSet(["olho","mao","pe","boca","voz","rosto","pele","cabelo","corpo","coracao","sangue","osso","pulso","respiracao","gesto","postura","olhar","toque","abraco","beijo","lagrima","suor"]);
  var SUBST_NATUREZA = new LSet(["terra","agua","vento","chuva","neve","fogo","pedra","areia","lama","barro","arvore","folha","raiz","flor","fruto","semente","mar","onda","correnteza","brisa","tempestade","trovao","relampago"]);
  var SUBST_EMOCAO   = new LSet(["amor","medo","raiva","alegria","tristeza","saudade","solidao","esperança","desespero","angustia","ansiedade","calma","paz","tensao","vergonha","orgulho","culpa","alívio","ternura","nostalgia"]);
  var SUBST_ABSTR    = new LSet(["vida","morte","tempo","silencio","verdade","mentira","justiça","liberdade","poder","memoria","destino","acaso","sorte","escolha","limite","possibilidade","impossibilidade","vazio","ausencia","presença"]);
  ...
  var DEFINICOES = {
    ...
    // Estrutura formal, versificação e linguística — v942
    "coda":          "A coda é a seção final de um texto musical ou literário — o momento de chegada, depois que o desenvolvimento principal se encerrou. Na ficção, a coda é frequentemente o capítulo ou trecho que vem depois do clímax principal e oferece um último olhar sobre as personagens e o mundo. Pode ser longa ou brevíssima. Uma boa coda não explica o que aconteceu: ela deixa a leitora sair da história com a sensação certa.",
    "preludio":      "O prelúdio é o que vem antes do começo — a abertura que prepara o terreno sem ser ainda a história. Na ficção, o prelúdio pode ser uma cena que não pertence à cronologia principal, uma voz que ainda não tem nome, um estado de mundo antes do conflito. O prelúdio bem escrito funciona como chave: só depois de ler o livro todo a leitora entende o que aquelas primeiras páginas estavam dizendo.",
    "interludio":    "O interlúdio na ficção é a pausa entre movimentos — um capítulo, seção ou trecho que suspende o fluxo principal para criar respiração, distância ou contraste. O interlúdio pode ser uma cena de outro ponto de vista, uma analepse, um poema inserido, uma carta. Ele funciona como a vírgula longa da estrutura: quando bem colocado, muda o ritmo sem quebrar a continuidade; quando mal colocado, interrompe o que estava vivo.",
    "periodo":       "O período na gramática portuguesa é a unidade sintática completa — a frase com todos os seus elementos. Na escrita literária, o período importa como unidade de ritmo: um período longo com muitas subordinadas cria desaceleração e peso; períodos curtos criam velocidade e urgência. A escritora que controla a extensão dos seus períodos controla o tempo da leitura. Ler o texto em voz alta revela o período que está comprido demais.",
    "oracao":        "A oração é a unidade sintática com verbo — o bloco de construção do período. Na gramática escolar, a oração é classificada por função (principal, subordinada, coordenada); na prática da escrita, a oração é a unidade de ação: o que acontece, quem faz, a quem acontece. A escritora que entende a estrutura das suas orações entende por que algumas frases ficam pesadas: frequentemente há orações demais encadeadas sem que cada uma precise existir.",
    "metro":         "O metro na poesia é a medida do verso — a contagem de sílabas poéticas que cria padrão rítmico. O decassílabo (dez sílabas) é o metro mais clássico da poesia lusófona; o alexandrino (doze sílabas) é o mais nobre da tradição francesa que influenciou o parnasianismo brasileiro. A poesia de metro fixo exige disciplina; a verso-librista pode ignorar o metro ou usá-lo como contraponto. Saber o metro que você está quebrando é diferente de não saber que ele existe.",
    "silaba":        "A sílaba é a unidade mínima de pronúncia — o pulso da língua. Na versificação, a contagem de sílabas poéticas segue regras específicas: a sílaba final átona não conta após a tônica final; encontros vocálicos podem se fundir (sinalefa) ou se separar (hiato). Para a prosa, a consciência da sílaba aparece no ritmo: palavras longas desaceleram, palavras curtas aceleram. A sílaba é onde a língua oral e a língua escrita se encontram.",
    "vocalulo":      "O vocábulo é a palavra considerada como unidade do léxico — com todas as suas formas flexionadas e derivadas. O vocabulário de um texto é o conjunto de vocábulos que a escritora usa, e esse conjunto revela escolhas: de registro (formal ou coloquial), de época (arcaico ou contemporâneo), de origem (latinismo, africanismo, indigenismo, neologismo). Ampliar o vocabulário não é usar palavras raras — é ter mais ferramentas para a precisão.",
    "morfema":       "O morfema é a menor unidade com significado na língua — menor que a palavra, maior que o fonema. Um vocábulo como 'escritoras' é formado por morfemas: 'escrit-' (raiz), '-or-' (agente), '-a-' (feminino), '-s' (plural). A consciência morfológica importa para a escritora quando ela trabalha com neologismos, com formações incomuns, com palavras que ela quer torcer. Entender morfemas é entender como a língua constrói sentido por composição.",
    "aliteracao":    "A aliteração é a repetição de sons consonantais no início de palavras próximas — recurso sonoro que cria musicalidade, ênfase ou efeito expressivo. 'Sete setas sibilam' é aliteração de 's'. Na prosa literária, a aliteração pode ser sutil e intencional ou acidental e infeliz. A aliteração acidental é um dos problemas que a leitura em voz alta revela mais facilmente: o chiado repetido que a escritora não ouviu ao escrever.",

    // Paisagem, formas poéticas e identidade de autoria — v955
    "paisagem":      "A paisagem na ficção é muito mais do que fundo decorativo — é estado interno externalizado, expressão de relação entre personagem e mundo, superfície onde o tempo histórico fica inscrito. Em Guimarães Rosa, o sertão é personagem. Em Conceição Evaristo, a favela é arquitetura afetiva. Em Lya Luft, a casa é a mente. A escritora que trata a paisagem só como cenário perde a metáfora mais rica que a ficção tem: a exterioridade que carrega interioridade.",
    "cenario":       "O cenário é o espaço imediato onde a cena acontece — o banheiro, a rua, o escritório. Diferente da paisagem (que carrega sentido simbólico mais amplo), o cenário é o lugar específico, com seus objetos, sua luz, seu cheiro, sua temperatura. Um bom cenário diz quem é a personagem sem que a autora precise dizer: uma escrivaninha vazia e uma lotada são dois personagens diferentes. O cenário é o primeiro dado que o leitor processa; por isso, a primeira frase que instala o cenário é uma das mais responsáveis do texto.",
    "exegese":       "A exegese é a leitura interpretativa minuciosa de um texto — originalmente bíblica, depois expandida para toda análise literária profunda. A exegese lê palavra por palavra, verso por verso, buscando o que o texto diz, o que implica e o que pressupõe. A exegese literária é o contrário da leitura impressionista: é leitura com responsabilidade com o texto, não com as reações de quem lê. Aprender exegese é aprender a ver o que está no texto antes de projetar o que não está.",
    "pseudonimo":    "O pseudônimo é o nome literário que a autora adota no lugar do nome civil — de George Sand a Clarice Lispector (que tinha nome de nascimento diferente), de Fernando Pessoa a bell hooks. O pseudônimo pode ser proteção (publicar sem o nome real), construção de persona literária distinta, ou resposta à discriminação (mulheres que publicavam sob nomes masculinos para serem levadas a sério). O pseudônimo coloca em questão quem é a 'autora' e o que significa assinar um texto.",
    "anacronismo":   "O anacronismo é a presença de algo fora do tempo correto — um objeto, uma palavra, um comportamento que não pertence à época em que a narrativa se passa. Na ficção histórica, o anacronismo involuntário é erro; o anacronismo intencional é recurso. Autoras que colocam anacronismos deliberados estão comentando o presente através do passado, ou mostrando que certos problemas não têm época. O anacronismo interroga a ideia de que cada tempo é separado dos outros.",
    "narradora":     "A narradora é a voz feminina que conta — não a autora, mas a persona narrativa construída no texto. A narradora em primeira pessoa ocupa um espaço historicamente negado às mulheres: o de quem conta a própria história a partir de si mesma, não a partir do olhar masculino. Escritoras como Virginia Woolf, Clarice Lispector e Toni Morrison criaram narradoras que recusaram o papel de objeto narrado. A narradora feminina é ato político e ato literário ao mesmo tempo.",
    "ghazal":        "O ghazal é uma forma poética árabe e persa — estrofes de dois versos (chamados shers), com rima obrigatória no segundo verso de cada par, e a autora mencionando o próprio nome na última estrofe. O ghazal chegou às literaturas do Oriente Médio, da Índia e do mundo urdu com Rumi, Hafez e Faiz Ahmed Faiz. Na literatura anglófona e brasileira contemporânea, o ghazal é usado como forma de experimentação com tradições não ocidentais. A restrição formal do ghazal, como a do soneto, cria soluções que a forma livre não encontraria.",
    "incorporacao":  "A incorporação, no contexto literário e performático, é o ato de habitar um personagem ou uma voz com o corpo inteiro — não só intelectualmente. Na dramaturgia, a incorporação é o trabalho do ator; na ficção, é o que acontece quando a escritora escreve a partir de dentro da personagem, não de fora. A incorporação literária tem um risco criativo: quando a escritora entra tão fundo que perde a distância. E uma recompensa: quando a personagem começa a fazer escolhas que a escritora não planejou.",
    "reciprocidade": "A reciprocidade na escrita é a relação de mútua influência entre o texto e quem o cria — a escritora transforma o texto, e o texto transforma a escritora. A reciprocidade também descreve a relação ideal com a leitora: não entrega passiva, mas troca ativa. No contexto das oficinas literárias, a reciprocidade é fundamento pedagógico: quem comenta um texto aprende tanto quanto quem o escreveu. A escrita sem reciprocidade — sem resposta, sem leitora, sem comunidade — é monólogo; com ela, é conversa.",
    "autopublicacao": "A autopublicação é a publicação de um livro pela própria autora, sem intermediação editorial. Com as plataformas digitais, a autopublicação deixou de ser estigma e passou a ser estratégia: escritoras que não conseguem ou não querem acesso às editoras tradicionais têm outros caminhos. A autopublicação exige que a autora assuma papéis que antes eram da editora: revisão, diagramação, distribuição, comunicação. Implica mais controle criativo e mais trabalho de gestão. No Brasil, escritoras independentes do Wattpad criaram audiências maiores do que muitas editadas por grandes casas.",
    "glosa":         "A glosa é a forma poética em que cada verso de um texto-base (geralmente de outro autor) vira o último verso de uma estrofe nova. A glosa é diálogo e homenagem ao mesmo tempo: a autora tece ao redor de cada verso uma nova construção que o conclui com novo sentido. A glosa ibérica medieval foi amplamente usada em língua portuguesa — e ressurge hoje como forma de revisitar o cânone, de conversar com o passado, ou de reescrever com ironia o que foi escrito com solenidade.",
    "lipograma":     "O lipograma é o texto que exclui intencionalmente uma ou mais letras — a restrição mais radical de todas as formas com restrição. O exemplo mais famoso é 'La Disparition' de Georges Perec (1969), que excluiu completamente a letra 'e' em francês — a letra mais comum da língua. O lipograma é exercício radical de atenção: escrever sem a letra certa força a língua a encontrar caminhos que nunca encontraria. O Oulipo (Ouvroir de Littérature Potentielle), fundado por Perec e Queneau, sistematizou o uso de restrições formais como método criativo.",

    // Narrador, cânone, tempo literário e espaço — v954
    "narrador":      "O narrador é a voz que conta — não confundir com a autora. O narrador é uma construção, uma persona que existe dentro do texto. O narrador pode saber tudo (onisciente), saber apenas o que uma personagem sabe (limitado), ou saber menos do que o leitor (ingênuo). A escolha do narrador é uma das decisões mais estruturais que a escritora toma: define o que pode ser dito, o que pode ser visto, e de onde vem a autoridade da história.",
    "interpretacao": "A interpretação é o ato de construir sentido a partir de um texto — não descobrir o que ele 'significa', mas produzir uma leitura possível dentro dos limites que o texto oferece. Toda interpretação é parcial: nenhuma leitura esgota o texto. A interpretação literária não é opinião aleatória: pressupõe evidências no próprio texto, conhecimento de contexto e responsabilidade com o que se afirma. Uma boa interpretação abre o texto; uma má, o fecha.",
    "canon":         "O cânone literário é o conjunto de obras e autoras consideradas fundamentais por uma tradição — as que 'todo mundo deveria ter lido'. O cânone foi historicamente construído por homens brancos europeus, e reflete os critérios de quem o selecionou. A crítica contemporânea não descarta o cânone: o questiona. Quem ficou de fora? Por quê? O cânone brasileiro está em disputa: a literatura negra, indígena e periférica exige seu lugar — não como complemento, mas como fundação alternativa.",
    "nostalgia":     "A nostalgia é o desejo de voltar a um lugar ou tempo que existe mais na memória do que na realidade — porque a memória já o transformou. A nostalgia literária não é saudosismo simples: pode ser crítica, pode ser irônica, pode ser o fio que conecta o presente ao que foi perdido ou ao que nunca existiu exatamente como é lembrado. Autoras que escrevem sobre infância, sobre lugares de origem, sobre períodos históricos lidam sempre com a distância entre o que foi e o que a escrita cria.",
    "novela":        "A novela é um gênero narrativo entre o conto e o romance — mais longa que o conto para desenvolver personagens e situações, mais curta que o romance para manter a tensão contínua sem dispersão. A novela clássica — Cervantes, Boccaccio, Kafka — tem uma unidade de ação que o romance pode fragmentar. No Brasil, 'novela' também remete à telenovela, o que cria um campo semântico complexo: a novela literária e a novela televisiva compartilham a serialidade, não necessariamente a profundidade.",
    "ambientacao":   "A ambientação é a construção do espaço onde a história acontece — não só descrição de lugar, mas a atmosfera, o tempo histórico, as condições sociais que tornam aquele mundo plausível. A boa ambientação não interrompe a narrativa para descrever: emerge das ações, das sensações e das percepções das personagens. A escritora que domina a ambientação cria mundos que o leitor pode habitar — não ilustrações, mas espaços com textura própria.",
    "autocritica":   "A autocrítica é a capacidade de avaliar o próprio trabalho com distância suficiente para ver o que não funciona — sem destruir o que funciona. A autocrítica precoce mata o rascunho: a escritora que avalia cada frase enquanto escreve paralisa. A autocrítica tardia é a ferramenta da revisão: depois do rascunho existir, olhar de fora e perguntar o que o texto precisa ser. A autocrítica honesta é rara — pede distância de si mesma que a maioria das escritoras conquista com tempo e leitura.",
    "rotina":        "A rotina de escrita é a estrutura de tempo que a escritora cria para que a escrita aconteça independente de inspiração. Muitas escritoras profissionais têm rotina rigorosa: horário fixo, lugar fixo, ritual de início. A rotina não é inimiga da espontaneidade — é o que a torna possível sem depender do acidente. Sem rotina, a escrita compete com tudo o que a vida oferece de mais urgente, e a vida oferece muitas urgências. A rotina declara: esta hora é da escrita, não da urgência.",
    "hermeneutica":  "A hermenêutica é a teoria da interpretação — originalmente bíblica, depois ampliada para toda leitura de textos. Hans-Georg Gadamer descreveu o 'círculo hermenêutico': para entender uma parte, precisa-se do todo; para entender o todo, precisa-se das partes. A hermenêutica literária pergunta como chegamos ao sentido, quais pressupostos carregamos para a leitura, e como o encontro entre texto e leitor produz algo que nenhum dos dois tinha antes. Toda leitura séria é um ato hermenêutico.",
    "cronotopo":     "O cronotopo é o conceito de Mikhail Bakhtin para a fusão de tempo e espaço na narrativa literária — a forma como cada gênero literário organiza essas duas dimensões de modo característico. O romance de estrada tem seu cronotopo (a viagem como transformação); o romance policial tem o seu (a investigação que desfaz o tempo, vai ao passado). O cronotopo não é só cenário: é a estrutura profunda que organiza o que pode acontecer, quando e onde. Cada gênero pensa o tempo e o espaço de modo diferente.",
    "topografia":    "A topografia narrativa é a configuração espacial do mundo da obra — a relação entre os espaços, o que está perto, o que está longe, o que é acessível e o que é proibido. A topografia não é apenas mapa: revela poder. Em Machado de Assis, os cortiços e as casas nobres são topografia de classe. Em Conceição Evaristo, a favela não é cenário — é perspectiva. A topografia da ficção reflete sempre a topografia do poder: quem pode mover-se pelo espaço da história e quem está preso em território.",
    "tanka":         "O tanka é a forma poética japonesa de 31 sílabas — 5/7/5/7/7 — mais antiga que o haiku, dela derivado. O tanka tem duas estrofes: os três primeiros versos (kami-no-ku) e os dois últimos (shimo-no-ku). A segunda estrofe responde, complementa ou subverte a primeira. O tanka permite mais desenvolvimento emocional do que o haiku — mais espaço para o sentimento, sem a objetividade extrema da imagem japonesa. No Brasil, poetas contemporâneos têm explorado o tanka como forma híbrida entre a tradição oriental e o verso livre.",

    // Leitura, crítica e formas breves — v953
    "leitor":        "O leitor é co-autora do texto: traz experiência, memória e expectativa que o texto não controla. Roland Barthes declarou a morte do autor para anunciar o nascimento do leitor — não como destino passivo, mas como produtor de sentido. A escritora que pensa no leitor não escreve para agradar: cria as condições para um encontro. O leitor implícito é o leitor imaginado dentro do próprio texto — e toda escolha de estilo é uma hipótese sobre quem vai ler.",
    "leitura":       "A leitura é uma atividade criativa — não recepção passiva, mas construção ativa de sentido a partir de marcas no papel. Cada leitura é única: o mesmo texto lido em momentos diferentes pela mesma pessoa produz experiências distintas. A escritora que lê muito aprende o que a análise não ensina: o ritmo do parágrafo, a temperatura da cena, o momento em que o texto perde ou ganha o leitor. Ler é o único método de formação que não pode ser substituído.",
    "releitura":     "A releitura revela o que a primeira leitura não pode ver: o que estava plantado desde o início, o que só faz sentido depois do fim, o que passou despercebido porque a tensão narrativa ocupava toda a atenção. Muitas obras se revelam completamente diferentes na segunda leitura — não porque o texto mudou, mas porque o leitor chegou com mais instrumentos. A escritora que relê os próprios textos encontra o que esqueceu de escrever e o que escreveu sem querer.",
    "critica":       "A crítica literária é a leitura especializada que analisa, contextualiza e avalia a obra — não para destruí-la, mas para entender o que ela faz e como faz. A boa crítica amplia o que é possível dizer sobre um texto; a má crítica reduz o texto ao que a crítica já sabia dizer. A escritora que lê crítica aprende a ver sua própria obra de fora — a distância crítica que o calor da escrita não permite. Toda crítica revela tanto sobre quem critica quanto sobre o criticado.",
    "autoria":       "A autoria é a relação de responsabilidade entre uma escritora e um texto — quem criou, quem responde, quem assina. A autoria não é simples: toda obra é construída sobre leituras de outros, sobre tradições absorvidas, sobre língua que já existia antes. Foucault perguntou 'O que é um autor?' — e a resposta ainda não é simples. A escritora contemporânea pode assinar um texto, mas sabe que ele atravessa outras vozes. A assinatura é uma convenção que protege um direito; a obra é sempre maior do que quem a assinou.",
    "escritora":     "A escritora é quem escreve — não como ocupação ocasional, mas como prática que organiza a vida e a atenção. A feminização do substantivo não é decorativa: apaga séculos em que 'escritor' era neutro masculino e as escritoras eram exceções. O Escrevaral usa 'escritora' não para excluir, mas para tornar visível quem foi invisibilizado. Ser escritora é uma escolha de atenção: decidir que o texto merece o tempo que o mundo não vai separar para você.",
    "cronica":       "A crônica é um dos gêneros mais brasileiros da literatura — curta, coloquial, enraizada no presente, publicada primeiro em jornal. De Machado de Assis a Clarice Lispector, de Carlos Drummond a Rubem Braga: a crônica brasileira é um laboratório de prosa onde o cotidiano vira literatura sem perder o chão. A crônica não é artigo de opinião: é prosa de invenção com pretexto no real. Quem aprende a escrever crônica aprende a ver o extraordinário no que parece ordinário.",
    "miniatura":     "A miniatura literária é a forma ultra-curta — de um parágrafo a uma página — em que toda a potência do conto precisa caber sem espaço para acumulação. A miniatura exige que a escritora elimine o que na forma longa seria desenvolvimento e deixe só o essencial. O silêncio da miniatura é estrutural: o que não está escrito é parte da obra. Lygia Fagundes Telles, João Gilberto Noll e Conceição Evaristo escreveram miniaturas que valem por romances inteiros.",
    "aforismo":      "O aforismo é a formulação breve e precisa de uma verdade geral — uma frase que condensa uma experiência ou um princípio de forma que não precisa de contexto para se sustentar. 'Feliz aquele que transfere o que sente para o que faz' (Guimarães Rosa) é aforismo: diz tudo em poucos termos e deixa o leitor com mais do que encontrou. O aforismo bem-feito parece sempre ter existido — como se não tivesse sido inventado, mas descoberto.",
    "haiku":         "O haiku é a forma poética japonesa de 17 sílabas — 5/7/5 — que busca capturar um instante de percepção entre o mundo humano e o mundo natural. Basho, Issa, Buson: os grandes haicaístas japoneses escreveram versos que ainda ressoam séculos depois. No Brasil, o haiku encontrou solo fértil — tanto na tradição da imigração japonesa quanto como forma de experimentação poética. O haiku não descreve: surpreende. Toda imagem boa de haiku produz algo que não estava na imagem.",
    "cartografia":   "A cartografia narrativa é o ato de mapear o espaço da ficção — não apenas descrever lugares, mas construir a lógica espacial que organiza o mundo da obra. Em Fantasy e ficção científica, a cartografia é literal: mapa do mundo inventado. Na ficção realista, é metafórica: a escritora sabe onde cada coisa acontece, como os espaços se conectam, quais territórios as personagens podem e não podem cruzar. Um mapa bem-feito é já uma forma de entender quem tem e quem não tem mobilidade na história.",
    "saudade":       "A saudade é um sentimento sem tradução perfeita — a presença da ausência, o amor pelo que não está, a nostalgia que não é tristeza e o desejo que não é espera. Em português, saudade é palavra-chave: Fernando Pessoa a definiu como 'a presença de uma ausência'; o Dicionário da Real Academia disse que é 'intraduível'. Na literatura brasileira, a saudade não é só sentimento: é uma forma de narrar o tempo — o presente carregado de passado, o futuro imaginado a partir do que foi perdido.",

    // Figuras, editorial e escuta — v952
    "disciplina":    "A disciplina criativa é a capacidade de escrever mesmo quando a inspiração não chegou — de sentar, abrir o texto e começar, mesmo que o início seja ruim. A disciplina não mata a espontaneidade: cria as condições para que ela apareça. A escritora que escreve só quando está inspirada escreve pouco. A que escreve todos os dias aprende a diferenciar os dias ruins dos dias em que o texto não quer se mostrar — e isso é uma distinção importante.",
    "catacrese":     "A catacrese é o uso desgastado de uma metáfora — tão repetido que já não soa como imagem, mas como sentido literal. 'Pé da mesa', 'cabeça de alho', 'braço do rio': ninguém mais visualiza partes do corpo humano ao usar essas expressões. A catacrese revela que toda linguagem é metáfora esquecida. Na ficção, a catacrese pode ser usada conscientemente — seja para marcar um registro coloquial, seja para ser reativada: fazer o leitor ver de novo o que parou de ver.",
    "zeugma":        "O zeugma é a figura em que uma palavra (geralmente um verbo) serve a dois termos ao mesmo tempo, mas se relaciona diferentemente com cada um — 'Saiu com o casaco e sem despedir-se.' O zeugma comprime sentido; exige que o leitor complete a relação. Na poesia, o zeugma cria efeito de simultaneidade. Na prosa, pode gerar ironia ou elipse elegante. O zeugma mal-feito é só omissão descuidada; o bem-feito é economia expressiva.",
    "anostico":      "O acróstico é o poema ou texto em que as letras iniciais de cada verso formam uma palavra, frase ou nome lido verticalmente. O acróstico é uma das formas mais antigas de jogo literário — aparece na Bíblia hebraica, em poetas medievais e em literatura contemporânea. A restrição do acróstico força a escritora a fazer escolhas que de outra forma não faria: a forma imposta cria soluções inesperadas.",
    "enunciacao":    "A enunciação é o ato de dizer — quem fala, para quem, em que condições, com que intenção. A teoria da enunciação (Benveniste, Ducrot) distingue o enunciado (o que foi dito) da enunciação (o ato de dizer). Na análise literária, a enunciação importa: o mesmo enunciado tem sentidos diferentes conforme quem enuncia, de onde enuncia e para quem. A escritora que pensa em enunciação pensa em posição — e posição é sempre política.",
    "escuta":        "A escuta ativa é, na escrita, a atenção ao que o texto pede — não o que a escritora quer que ele seja. Escrever bem começa por ouvir bem: a língua falada ao redor, os ritmos que surgem naturalmente, o que os personagens diriam se não fossem controlados. A pedagogia de Paulo Freire começa pela escuta; a de muitas oficinas literárias contemporâneas também. Ouvir é anterior ao narrar: sem escuta, a ficção se torna projeção, não descoberta.",
    "presenca":      "A presença na performance literária é a qualidade de quem ocupa o espaço e o tempo com convicção — a voz que carrega peso, o olhar que sustenta. Na ficção, a presença narrativa é a sensação de que há alguém por trás do texto — uma consciência que escolheu cada palavra e que responde pela escolha. A escritora que perde a presença no próprio texto escreve de forma displicente; a que tem excesso de presença pode sufocar o espaço do leitor.",
    "ressonancia":   "A ressonância é o efeito de um texto que continua vibrar depois de terminado — que ecoa, que volta, que muda de sentido com o tempo. Um texto ressoa quando plantou algo que o leitor vai encontrar no mundo e que vai remeter de volta ao livro. A ressonância não é calculada: é consequência de escolhas que tocam algo mais fundo do que a intenção consciente da autora. Textos que ressoam são os que se lembram décadas depois.",
    "orelha":        "A orelha é o texto que aparece na dobra da capa de um livro — geralmente uma nota sobre a obra e uma nota sobre a autora. A orelha precisa seduzir sem revelar demais: é o último texto que o leitor lê antes de decidir comprar. Uma boa orelha não descreve o livro — instala uma expectativa. Muitas escritoras escrevem a orelha do próprio livro; é um exercício de síntese e de distância: como apresentar ao leitor o que você não consegue mais ver de fora.",
    "ato":           "O ato é a divisão maior de uma peça teatral — equivale ao capítulo na ficção, mas com função estrutural mais rígida. A divisão em atos organiza o tempo dramático: cada ato tem seu arco, sua virada, seu estado final diferente do inicial. A ficção contemporânea emprestou do teatro a lógica dos atos sem a nomenclatura: muitos romances têm três ou cinco movimentos com a mesma força de virada que um ato bem construído.",
    "pausa":         "A pausa na escrita é o silêncio intencional — o espaço em branco entre parágrafos, o ponto e vírgula que suspende, a linha que não se escreve. A pausa musical é o lugar onde o som ressoa sem ser produzido; na prosa, a pausa é onde o sentido continua sem palavras. A escritora que nunca para não deixa o texto respirar. Aprender a fazer pausa é aprender que o leitor precisa de espaço para completar o que o texto deixou em aberto.",
    "editoria":      "A editoria é a área de uma editora responsável pela seleção, desenvolvimento e publicação dos livros — inclui a editora de texto, que trabalha com a autora no manuscrito, e a coordenadora editorial, que cuida do processo. A relação entre escritora e editora pode ser um dos vínculos mais frutíferos da vida literária — ou uma das mais frustrantes. Entender como funciona a editoria é entender quem toma as decisões sobre o que chega ao leitor.",

    // Processo criativo, publicação e dramaturgia — v951
    "rascunho":      "O rascunho é a primeira forma do texto — imperfeito, incompleto, vivo. A escritora que tem medo de rascunhar está confundindo o ato de descobrir com o ato de publicar. O rascunho não precisa ser bom: precisa existir. Muitas escritoras bloqueadas não precisam de inspiração — precisam de permissão para fazer algo ruim que depois vira algo melhor. O rascunho é o lugar onde a escrita ainda pode ser outra coisa do que você planejou.",
    "revisao":       "A revisão é o momento em que a escritora se torna leitora do próprio texto — com alguma distância e um olhar diferente do que criou. Revisar não é corrigir: é reconstruir. Uma revisão bem-feita pode mudar o sentido de um parágrafo inteiro mudando só uma palavra. A revisão é onde a maioria dos textos realmente nasce — o rascunho é a pedra bruta, a revisão é o entalhe. Escritoras que não revisam publicam rascunhos.",
    "bloqueio":      "O bloqueio criativo é a incapacidade temporária de iniciar ou continuar um texto — não por falta de talento, mas frequentemente por excesso de expectativa. A escritora que espera que o texto saia perfeito da primeira vez bloqueia antes de começar. O bloqueio é mais sintoma do que causa: pode ser medo do julgamento, exaustão, perfeccionismo ou simplesmente a necessidade de descanso que o corpo disfarça de paralisia criativa.",
    "inspiracao":    "A inspiração é o momento em que uma ideia chega de forma inesperada — o insight, a imagem, a frase que aparece sem ser convocada. A tradição romântica mitificou a inspiração como visita das musas; a tradição artesanal a trata como resultado de trabalho consistente que cria condições para o inesperado. A verdade está entre as duas: a inspiração existe, mas a escritora que espera por ela sem escrever vai esperar para sempre.",
    "sinopse":       "A sinopse é o resumo da obra para fins editoriais — geralmente entre 10 e 30 linhas, descrevendo personagens principais, conflito central e resolução. A sinopse não é um spoiler: é uma ferramenta de comunicação entre escritora e editora. Escrever uma boa sinopse é um exercício doloroso porque obriga a dizer o que o livro é de fato, sem o escudo do estilo. Muitas escritoras descobrem que o livro tem problemas estruturais quando tentam sintetizá-lo.",
    "resenha":       "A resenha é a análise crítica de um livro publicado — descritiva e avaliativa ao mesmo tempo. A boa resenha localiza a obra em um contexto (de gênero, de tradição, de momento editorial), descreve o que o livro faz, e avalia se o faz bem ou mal, e por quê. A resenha é uma forma de atenção literária: saber ler resenhas ajuda a escritora a entender como seus próprios textos podem ser recebidos e discutidos.",
    "cena":          "A cena é a unidade básica da narrativa dramática — um segmento de ação com personagens, espaço e conflito identificáveis. Em ficção, a cena equivale à unidade de atenção: o leitor experimenta tempo e espaço em tempo presente, como se estivesse vendo. Cada cena deve transformar algo: o estado de uma personagem, a relação entre dois personagens, o que o leitor sabe. Uma cena que termina exatamente como começou não precisa estar no livro.",
    "prologo":       "O prólogo é o texto que antecede a narrativa principal — pode ser uma cena anterior aos eventos do livro, uma voz que fala de fora da trama, ou uma informação que o leitor precisa ter antes de entrar. O prólogo bem-feito cria um contexto sem revelar demais, instala um tom, ou levanta uma pergunta que o livro vai demorar a responder. O prólogo mal-feito é uma desculpa para a autora se explicar antes de começar.",
    "epilogo":       "O epílogo é o texto que vem depois da narrativa principal — um salto no tempo, uma coda, um eco. O epílogo pode mostrar como ficaram os personagens anos depois, fechar uma promessa que o livro fez cedo, ou introduzir uma ambiguidade de última hora que muda o sentido do que o leitor leu. O bom epílogo não explica: ressoa. O mau epílogo desfaz o efeito da conclusão ao insistir em amarrar pontas que eram mais interessantes abertas.",
    "dialogo":       "O diálogo na ficção não é transcrição de conversa real — é conversa destilada, com intenção, subtexto e ritmo. Toda fala de personagem faz algo além de transmitir informação: revela caráter, avança conflito, constrói relação. O diálogo sem subtexto é plano; o diálogo todo subtexto é hermético. A escritora que aprende a escrever diálogo aprende a escutar o que não se diz — o silêncio entre as falas frequentemente conta mais do que as palavras.",
    "silencio":      "O silêncio na ficção é tão construtivo quanto a fala — é o que não é dito, o que a personagem não consegue nomear, o espaço vazio entre dois parágrafos onde o leitor projeta o que o texto não ousou dizer. O silêncio narrativo pode ser escolha estética (a elipse, o corte) ou psicológica (a personagem que cala algo). A escritora que aprende a usar o silêncio aprende que a ausência pode ser mais carregada do que a presença.",
    "voz":           "A voz na ficção é a personalidade do texto — não só o que narra, mas como narra, com que ritmo, com que vocabulário, com que distância. Toda escritora tem uma voz que está se formando ou se consolidando; ela emerge do que foi lido, do que foi vivido, e das escolhas que o texto repetiu até virarem hábito. A voz não é impostação: é reconhecimento. Quando uma escritora encontra a própria voz, os textos passam a soar como dela — e não poderiam ser de mais ninguém.",

    // Rito, sagrado, sobrevivência e resistência — v950
    "ressignificacao": "Ressignificar é atribuir novo sentido a algo sem apagar o sentido anterior — é trabalhar com a memória do signo. Na literatura, ressignificar uma palavra, um símbolo ou uma tradição é um ato de poder: quem ressignifica não aceita o sentido herdado, mas também não o ignora. O vocabulário da escritura negra e indígena no Brasil está cheio de ressignificações — palavras que foram usadas para ferir e foram retomadas como grito, orgulho, literatura.",
    "ritual":         "O ritual é a ação simbólica repetida que marca passagem, pertencimento ou contato com o sagrado — o batismo, o enterro, a festa, a prece, o silêncio coletivo. Na ficção, o ritual é estrutura e metáfora: organiza tempo e comunidade, sinaliza o que é irreversível. Quando uma personagem participa ou rompe um ritual, algo muda na ordem da história. A escritora que entende o ritual entende que há ações que o enredo não pode desfazer.",
    "sagrado":        "O sagrado é aquilo que está separado do cotidiano por ser fonte de poder, de sentido ou de perigo — o nome que não se pronuncia, o lugar que não se pisa, o objeto que não se toca. Mircea Eliade definiu o sagrado em oposição ao profano: dois modos de ser no mundo, não dois lugares. Na ficção brasileira, o sagrado frequentemente emerge das religiões afro-brasileiras, indígenas e do catolicismo popular — e qualquer dessas tradições merece ser narrada de dentro, não de fora.",
    "profano":        "O profano é o oposto do sagrado — não no sentido de maldito, mas de ordinário, de mundo cotidiano, de tempo linear sem transcendência. O profano não é inferior ao sagrado: é o chão onde a vida acontece, onde personagens comem, trabalham, brigam e amam. A tensão entre sagrado e profano é uma das mais antigas da narrativa — o momento em que o cotidiano é atravessado pelo incomum, pelo rito, pelo invisível.",
    "sobrevivencia":  "A sobrevivência na ficção não é só escapar da morte: é o estado de quem continua depois de uma ruptura que deveria ter encerrado tudo. A personagem sobrevivente carrega o peso de testemunhar — e frequentemente também a culpa. A literatura de sobrevivência está no coração da ficção pós-catástrofe, da literatura do cárcere, do testemunho do exílio. Escrever sobre sobrevivência é sempre escrever sobre o que não sobreviveu junto.",
    "resiliencia":    "A resiliência, no contexto literário, é a capacidade de uma personagem, de uma comunidade ou de uma tradição de se dobrar sem se partir — e de retornar transformada. O conceito vem da física (materiais que voltam à forma após deformação) e foi apropriado pela psicologia e pela cultura. Na ficção, a resiliência não é virtude simples: a escritora que retrata personagens resilientes precisa mostrar o custo do dobramento, não só a volta.",
    "deslocamento":   "O deslocamento é o movimento — físico, afetivo, cultural — que tira um ser de seu lugar de origem. Na ficção, o deslocamento não é só partida geográfica: é a perda do enquadramento familiar, da língua de casa, do referencial que dava sentido. Personagens deslocadas precisam construir novo mapa enquanto ainda estão perdidas. A literatura de deslocamento é um dos gêneros mais urgentes do século XXI — nas migrações, nos refúgios, nos exílios internos.",
    "enraizamento":   "O enraizamento é a condição de quem tem chão — terra, língua, comunidade, tradição que sustenta. Édouard Glissant distinguiu o enraizamento como identidade única e fechada (a raiz que exclui) da rizoma como identidade múltipla e aberta. Na ficção brasileira, o enraizamento aparece como tensão: as culturas indígenas que reclamam território, as comunidades quilombolas que guardam memória, o sertão que não deixa ir.",
    "subversao":      "A subversão é o movimento de inverter ou minar uma ordem estabelecida — não pelo confronto direto, mas pela torção do sentido de dentro. A literatura subversiva usa a forma do poder contra o poder: a escritora que narra como a norma espera, mas semeia uma perspectiva que a dissolve. A subversão literária não precisa ser explícita: às vezes está no ponto de vista, na escolha de quem conta, no que é deixado sem resposta.",
    "contranarativa": "A contranarativa é a história que contradiz, desloca ou expõe os limites da narrativa dominante. Toda grande literatura é, em algum grau, contranarativa — desafia o que se conta como natural, inevitável, universal. A contranarativa não é simples negação: constrói outra versão com tanta força interna quanto a versão que contesta. No Brasil, a literatura negra, a literatura indígena e a literatura periférica funcionam como contranarativas ao centro.",
    "cosmogonia":     "A cosmogonia é o mito da criação — a narrativa de como o mundo começou, de onde veio o ser humano, quais forças fundaram a existência. Cada cultura tem sua cosmogonia: o Gênesis do judaico-cristão, o Popol Vuh maia, os mitos de criação de povos indígenas brasileiros. Na ficção, evocar uma cosmogonia é invocar a origem mais funda do que a história permite. A escritora que trabalha com cosmogonias sabe que está tocando em algo que precede a narrativa.",
    "liminaridade":   "A liminaridade é o estado de quem está no limiar — entre dois mundos, dois estados, duas identidades. O conceito vem de Victor Turner, a partir de van Gennep: nos ritos de passagem, existe uma fase liminar onde o iniciando não é mais o que era e ainda não é o que será. Na ficção, personagens em estado liminar são as mais interessantes: carregam dois tempos ao mesmo tempo. O adolescente, o migrante, o sobrevivente, o convertido — todos habitam o limiar.",

    // Diáspora, exílio, pertencimento e transgresão — v949
    "traducao":      "A tradução literária é o ato de recriar um texto em outra língua — não transpor palavras, mas refazer sentidos, ritmos, silêncios. Traduzir é interpretar: toda escolha do tradutor é uma leitura. A tradução literária boa revela o que o original não sabia de si mesmo. Há algo na tradução que o original não tem: o contato com o estranho. A escritora que traduz aprende a desmontar e remontar a própria língua.",
    "palimpsesto":   "O palimpsesto é o manuscrito medieval de onde se raspou um texto para escrever outro por cima — e o antigo reaparece. Na literatura, toda obra é palimpsesto: texto sobre texto, escrita sobre leitura, voz sobre silêncio. Gerard Genette usou a palavra para descrever a intertextualidade: sob todo livro há outros livros parcialmente apagados. A escritora que reconhece seu palimpsesto sabe de onde veio e pode escolher o que deixar ver.",
    "arquetipo":     "O arquétipo é uma figura, padrão ou imagem que aparece em culturas e épocas distintas com a mesma carga simbólica — o herói, a sombra, a grande mãe, o trapaceiro. Carl Gustav Jung identificou arquétipos no inconsciente coletivo; Joseph Campbell rastreou-os nos mitos do mundo. Na ficção, personagens arquetípicas reconhecem algo que o leitor já carrega. O risco do arquétipo é se tornar estereótipo: a escritura precisa encarnar o padrão, não apenas citar.",
    "diaspora":      "A diáspora é o espalhamento de um povo além de seu território de origem — por exílio, escravidão, migração forçada ou deslocamento econômico. A literatura da diáspora é uma literatura do entre-lugar: quem escreve a partir da diáspora pertence a dois ou mais mundos e não pertence completamente a nenhum. No Brasil, a diáspora africana é fundamento — não pano de fundo. A literatura brasileira contemporânea que ignora a diáspora está falando de um Brasil que não existe.",
    "exilio":        "O exílio é a separação forçada de um lugar ao qual se pertence — pode ser geográfico, político, linguístico, cultural. Na literatura, o exílio produz uma relação intensa com a memória: quem está fora precisa reconstruir em palavras o que não pode tocar. Escritoras em exílio muitas vezes desenvolvem uma atenção especial à língua — o único território que levaram consigo. O exílio também pode ser interior: a escritora que não se reconhece em nenhuma tradição.",
    "mito":          "O mito é uma narrativa fundadora que explica a origem, o sentido e a ordem do mundo — não pela razão, mas pela imagem e pelo símbolo. Antes de ser folclore, o mito era verdade: a forma como uma comunidade entendia a si mesma e seu lugar no cosmo. Na literatura, recorrer ao mito é acionar uma memória coletiva mais funda do que a história. O mito brasileiro inclui o da democracia racial, que a escritura contemporânea tem desmontado com urgência.",
    "testemunho":    "A literatura de testemunho nasce do imperativo de contar o incontável — o genocídio, a tortura, a violência de estado, a catástrofe. Quem testemunha não está apenas narrando: está preservando o que não pode ser apagado. Primo Levi, Conceição Evaristo e Carolina Maria de Jesus são, cada um a seu modo, escritoras do testemunho. A literatura de testemunho coloca em crise a ficção pura: não porque seja mais verdadeira, mas porque o peso ético de cada palavra é diferente.",
    "hibridismo":    "O hibridismo cultural é a condição de quem existe entre duas ou mais culturas — não como síntese harmônica, mas como tensão viva. Homi Bhabha desenvolveu o conceito para analisar culturas colonizadas que não reproduzem nem o modelo colonizador nem o pré-colonial: criam algo terceiro, instável, fértil. A literatura brasileira é profundamente híbrida — de língua, de ritmo, de referência. Negar o hibridismo é negar a própria textura do país.",
    "pertencimento": "O pertencimento é o sentimento de ter um lugar — na família, na comunidade, no território, na língua, na tradição. Na ficção contemporânea, o pertencimento é frequentemente fraturado: personagens que não se encaixam, que chegam tarde ou saem cedo, que vivem no limiar. A escritora que escreve sobre pertencimento raramente celebra: investiga. Pertencer ou não pertencer é sempre uma questão de poder — quem decide quem fica dentro e quem fica de fora.",
    "transgressao":  "A transgressão literária é o ato de cruzar um limite que o texto sabe que existe — de linguagem, de forma, de tema, de propriedade narrativa. A transgressão não é desobediência aleatória: pressupõe que a escritora conhece a norma e escolhe ultrapassá-la com intenção. Marquis de Sade transgrediu pela violência; Clarice Lispector, pelo silêncio; Carolina de Jesus, pela existência. A transgressão que não sabe o que está transgredindo é só ruído.",
    "marginalidade": "A marginalidade literária é a condição de escritoras que produzem fora dos centros de legitimação — fora das editoras, das universidades, das listas de premiação. A literatura marginal brasileira, a partir dos anos 1970, reconfigurou quem pode narrar e de onde. Ferréz, Alessandro Buzo e Paulo Lins escrevem de dentro de territórios que a literatura oficial descreve de fora — e a diferença é estrutural. A escritora marginal não busca ser aceita pelo centro: redefiniu onde fica o centro.",
    "limiar":        "O limiar é o ponto de passagem entre dois estados — a soleira da porta, o instante antes da escolha, o momento em que a personagem ainda pode voltar. O conceito de liminaridade, de Victor Turner, descreve o estado de quem está entre: nem mais o que era, ainda não o que vai ser. Na ficção, os momentos de limiar são frequentemente os mais carregados: o personagem parado na entrada ainda não entrou na história que vai mudar tudo.",

    // Técnica narrativa, voz e relação com outros textos — v946
    "montagem":      "A montagem na ficção é a técnica de justaposição de elementos heterogêneos para criar sentido por contraste ou ressonância — emprestada do cinema (Eisenstein), adaptada para a prosa. A montagem literária não narra: coloca lado a lado. O leitor monta o sentido a partir do que foi yuxtaposto. A ficção que usa montagem abdica da explicação: confia que o impacto dos fragmentos juntos diz mais do que qualquer narrador diria.",
    "justaposicao":  "A justaposição é o recurso de colocar dois elementos lado a lado sem explicar a relação entre eles — deixando que o contato crie o sentido. A justaposição literária pode ser de cenas, de vozes, de tempos, de registros. Quando bem executada, a justaposição cria uma terceira coisa que nenhum dos dois elementos teria sozinho. É um dos recursos mais silenciosos e mais poderosos da prosa contemporânea.",
    "collage":       "A colagem literária é a técnica de incorporar ao texto materiais de outras origens — recortes de jornal, trechos de outros livros, cartas, receitas, documentos. A colagem rompe a ilusão da voz única e mostra a ficção como construção a partir de fragmentos de realidade. Autoras como Hilda Hilst e Flávio de Carvalho usaram a colagem como princípio estrutural. A colagem pergunta: de onde vêm os materiais da literatura? A resposta é sempre: de outros textos.",
    "macguffin":     "O MacGuffin é o objeto ou objetivo que os personagens perseguem e que move a trama — mas que em si não tem importância real. O termo foi popularizado por Alfred Hitchcock: o que está na mala, o que está no cofre, o segredo que todos querem saber. O MacGuffin é dispositivo narrativo puro: existe para colocar personagens em movimento, não para ser interessante por si mesmo. A ficção que confunde MacGuffin com tema central tem um problema.",
    "heteroglossia": "A heteroglossia é o conceito de Mikhail Bakhtin para a coexistência de múltiplas vozes, registros e linguagens dentro de um mesmo texto. O romance é o gênero heteroglóssico por excelência: nele, o discurso do narrador convive com os discursos das personagens, com os registros sociais, com as línguas de classe e época. A heteroglossia opõe-se ao monologismo: um texto que só tem uma voz, que apaga as diferenças de perspectiva, que fala como se houvesse apenas um ponto de vista.",
    "discurso":      "O discurso na análise literária é mais do que fala: é o modo como o poder se organiza através da linguagem — o que pode ser dito, quem pode dizer, em que condições. Foucault desenvolveu a análise do discurso como ferramenta crítica: discursos médicos, jurídicos, literários não descrevem a realidade, constroem-na. Para a escritora, entender discurso é entender que a língua não é neutra — cada palavra carrega uma história de quem a usou, para quê e contra quem.",
    "deuteragonista":"O deuteragonista é a segunda personagem mais importante da narrativa — o papel que no drama grego ficava logo após o protagonista. Na ficção contemporânea, o deuteragonista é frequentemente mais interessante do que o protagonista: tem menos pressão de ser herói, mais liberdade de ser contraditório. O deuteragonista que rouba a cena é sintoma de que a escritora encontrou algo mais rico do que planejava.",
    "parodia":       "A paródia é a imitação de um texto ou estilo com intenção cômica, crítica ou subversiva. A paródia pressupõe conhecimento: para parodiar, é preciso conhecer bem o que se está parodiando. A paródia literária pode ser carinhosa (homenagem cômical) ou corrosiva (desmontagem de um cânone). Dom Casmurro é parodiado há décadas pela crítica feminista; a paródia crítica é uma das formas mais eficazes de leitura ativa.",
    "homenagem":     "A homenagem literária é o texto que declara dívida — que reconhece explicitamente uma influência, um predecessor, uma tradição. A homenagem pode ser direta (dedicatória, epígrafe) ou estrutural (a forma do texto replica a de outro). A homenagem honesta não é imitação: é diálogo. A escritora que homenageia sabe o que está tomando e o que está transformando. Homenagear bem é prova de leitura profunda.",
    "adaptacao":     "A adaptação é a tradução de um texto de um meio ou forma para outro — do romance para o cinema, da peça de teatro para a ficção, da mitologia para o romance contemporâneo. A adaptação não é cópia: é releitura. A adaptação revela o que no original pode mudar sem que o essencial se perca — e o que não pode mudar sem que o original deixe de existir. As adaptações brasileiras de mitos gregos, bíblicos e africanos são um laboratório de como culturas se apropriam umas das outras.",

    // Identidade coletiva, circulação e coautoria — v945
    "etnia":         "A etnia na ficção é a identidade de pertencimento a um grupo com história, cultura e língua compartilhadas — e que frequentemente inclui uma experiência de opressão ou resistência coletiva. A ficção que narra personagens étnicas com profundidade não as trata como representantes de um grupo, mas como indivíduos dentro de um grupo: marcadas por ele mas não reduzidas a ele. A escrita da etnia no Brasil é política e literária ao mesmo tempo.",
    "nacao":         "A nação na ficção é tanto o espaço físico de um país quanto a ideia de pertencimento que esse espaço sustenta — e as ficções que sustentam essa ideia. Benedict Anderson chamou as nações de 'comunidades imaginadas': não naturais, mas construídas por narrativas compartilhadas. O romance histórico brasileiro frequentemente examina o que significa ser brasileiro, quais histórias a nação oficial silenciou, quais precisam ser reescritas.",
    "povo":          "O povo na ficção é a categoria coletiva mais ampla e mais usada para nomear quem costuma ser excluído das narrativas de poder. 'O povo' pode ser sujeito de uma história ou objeto de uma narrativa que o fala sem ouvi-lo. A ficção comprometida com o povo — no sentido de que se propõe a narrar de dentro, não de fora — tem uma tradição longa no Brasil: do regionalismo do século XX à literatura periférica contemporânea.",
    "coletivo":      "O coletivo na ficção é o grupo que age junto — que tem uma história comum, um projeto compartilhado, uma identidade forjada na relação. O coletivo literário tem voz própria que não é simplesmente a soma das vozes individuais. A ficção coral, a narrativa de perspectiva múltipla e o romance de geração são formas de dar voz ao coletivo sem apagar os indivíduos. O coletivo como personagem é um dos desafios mais interessantes da narrativa.",
    "datiloscrito":  "O datiloscrito é o texto produzido em máquina de datilografar — o original antes da digitalização, o manuscrito do século XX. O datiloscrito tem uma presença física que o arquivo digital não tem: páginas que amarelam, correções a tinta, a marca física do pensamento. Na ficção, o datiloscrito pode ser objeto narrativo: o manuscrito encontrado, o texto perdido, o último trabalho de uma escritora. É também metáfora de uma temporalidade específica da escrita.",
    "circulacao":    "A circulação na ficção é como os textos se movem — de mão em mão, de comunidade em comunidade, de uma época a outra. A circulação formal é a do mercado editorial; a circulação informal é a dos xerox, dos zines, dos grupos de WhatsApp, dos PDFs enviados por e-mail. A ficção que circula fora dos canais oficiais muitas vezes alcança leitoras que os canais oficiais não alcançam. A história da literatura brasileira inclui muita circulação que nunca virou livro.",
    "boletim":       "O boletim é o texto de circulação periódica e comunitária — produzido para manter uma comunidade informada, conectada, organizada. Na tradição de movimentos sociais brasileiros, o boletim é espaço de escrita coletiva e de formação de escritoras que nunca pensaram em si mesmas como tal. A ficção que inclui boletins como documentos internos à narrativa está incluindo uma história de escrita que os arquivos literários oficiais raramente guardam.",
    "panfleto":      "O panfleto é o texto de circulação rápida e intenção persuasiva — o formato político por excelência desde a Revolução Francesa. A escrita panfletária tem má reputação literária porque privilegia argumento sobre forma, urgência sobre permanência. Mas há panfletos que são também literatura: 'Abolicionismo' de Joaquim Nabuco começa como argumento e chega à prosa que dura. O panfleto lembra que a escrita sempre teve função no mundo além da contemplação.",
    "coautoria":     "A coautoria na ficção é a escrita a duas ou mais vozes — que implica negociação, divisão, renúncia e síntese. A coautoria literária é rara porque o processo de escrever ficção exige uma voz interior singular; mas quando funciona, produz algo que nenhuma das autoras produziria sozinha. A questão da coautoria também é política: quem assina, quem fica de fora, quem é creditada e quem desaparece na obra final.",
    "colaboracao":   "A colaboração na criação literária é mais ampla do que a coautoria: inclui a editora que molda o texto, o grupo de leitura que o desafia, a pesquisadora que fornece o material, a comunidade que torna possível a escrita. A ficção é sempre mais colaborativa do que a figura solitária da escritora sugere. Reconhecer a colaboração não diminui a autoria: expande o entendimento de como textos são feitos e por quê chegam ao que chegam.",

    // Espaço interior/exterior, tempo e vestígio — v944
    "interior":      "O interior na ficção tem dois sentidos que se cruzam: o espaço físico de dentro (o quarto, a casa, o porão) e o espaço psicológico de dentro (a vida mental de uma personagem). A ficção de interioridade é aquela que narra o que acontece por dentro — os pensamentos, os impulsos, as contradições que a personagem não diz. O romance moderno expandiu o espaço interior da personagem tanto quanto o século XX expandiu os recursos narrativos para acessá-lo.",
    "exterior":      "O exterior na ficção é o espaço da exposição — onde a personagem se relaciona com o mundo, onde é vista, onde precisa performar uma versão de si mesma. O exterior revela por contraste: o que a personagem faz lá fora versus o que pensa por dentro. A ficção que só narra o exterior tende ao superficial; a que só narra o interior tende ao solipsismo. A tensão entre os dois é onde muitas das histórias mais interessantes acontecem.",
    "limiar":        "O limiar é o espaço da passagem — o corredor entre dois mundos, o umbral que divide o antes do depois. Na mitologia e no folclore, o limiar é sagrado e perigoso: cruzá-lo muda quem cruza. Na ficção literária, o limiar aparece como o momento em que uma personagem está prestes a mudar e ainda não mudou — suspensa entre identidades. O anthropólogo Victor Turner chamou esse estado de 'liminaridade': quando velhos status caiu e o novo ainda não se firmou.",
    "encruzilhada":  "A encruzilhada na ficção é o ponto de decisão — onde dois ou mais caminhos se oferecem e apenas um pode ser tomado. A encruzilhada literária é também símbolo: nas tradições afro-brasileiras, Exu governa as encruzilhadas — o orixá da comunicação, das trocas, das escolhas. Uma personagem na encruzilhada está no ponto de maior tensão narrativa: ainda pode virar para qualquer lado. O que ela escolhe revela quem ela é.",
    "amnesía":       "A amnésia na ficção é um dos dispositivos narrativos mais explorados — e com razão: o personagem que não sabe quem é força a história a construir identidade de dentro para fora, em vez de declarar. A amnésia literária bem usada não é artifício de suspense: é exploração genuína de como a identidade é construída por memória, e do que sobra quando a memória vai. O risco é o clichê do thriller; a oportunidade é a pergunta filosófica séria.",
    "recordacao":    "A recordação na ficção é a memória ativa — o ato de trazer de volta, de tentar recuperar o que foi. A recordação é diferente da memória passiva: ela implica esforço, busca, às vezes sofrimento. A personagem que recorda está tentando segurar algo que escorrega. A recordação literária é frequentemente imperfeita: o que se lembra não é o que aconteceu, mas uma versão filtrada pelo tempo, pelo desejo e pelo medo. Isso é narrativamente rico, não problema.",
    "cura":          "A cura na ficção é um dos finais mais difíceis de escrever com honestidade — porque a cura real raramente é completa, limpa ou permanente. A ficção que oferece cura fácil trai a experiência de quem sofreu; a que nega qualquer cura trai a resiliência humana. A cura literária mais verdadeira é a cura parcial: a personagem que aprendeu a viver com o que não vai desaparecer, que reorganizou a vida em torno de uma perda que ficou.",
    "ruina":         "A ruína na ficção é o que sobrou depois que algo se desfez — e o que sobrou ainda conta uma história. A ruína de uma casa diz sobre quem viveu nela; a ruína de um relacionamento, sobre o que um dia foi possível. A ficção que narra ruínas está narrando o tempo — o que o tempo faz com as coisas e com as pessoas. A estética da ruína tem uma longa tradição: do romantismo europeu às favelas que a ficção brasileira contemporânea transforma em matéria literária.",
    "vestigio":      "O vestígio na ficção é a prova de que algo aconteceu — a marca que ficou depois que o fato passou. O vestígio é o que permite reconstruir o passado: o rastro de uma passagem, o objeto esquecido, a palavra que alguém disse e que ficou na memória de outra personagem. A ficção policial se organiza inteiramente em torno do vestígio; a ficção histórica também. Mas o vestígio aparece em qualquer narrativa que lida com o que não se pode mais testemunhar diretamente.",
    "rastro":        "O rastro na ficção é o vestígio em movimento — o que uma personagem deixa ao passar, o que indica sua direção, o que permite seguir ou perder seu caminho. O rastro literário pode ser físico (pegadas, objetos) ou imaterial (rumores, memórias de quem a conheceu). A escritora que usa o rastro como recurso narrativo está construindo presença através de ausência: a personagem se define pelo que deixou em quem ficou.",

    // Ciclos de vida e ritos de passagem — v943
    "nascimento":    "O nascimento na ficção é raramente sobre o recém-nascido — é sobre quem está ao redor. O nascimento literário transforma os outros: a mãe que se torna mãe, o pai que se confronta com uma responsabilidade que não esperava, a família que reorganiza seus centros. O nascimento também é metáfora: do texto que nasce, da personagem que começa a existir para si mesma, do projeto que deixa de ser ideia e vira realidade.",
    "transicao":     "A transição na ficção é o espaço entre dois estados — nem o que era antes nem o que vai ser depois. A personagem em transição está desorientada por definição: não pertence mais ao lugar de onde saiu e ainda não chegou ao lugar para onde vai. A ficção que narra transições bem as sustenta no tempo: não resolve cedo demais, não deixa a personagem num limbo inerte. A transição tem seu próprio ritmo.",
    "ritual":        "O ritual na ficção é o gesto que transforma — que marca uma passagem, que separa um tempo de outro, que inscreve um acontecimento no corpo ou na comunidade. O ritual literário não precisa ser religioso: pode ser o café da manhã que sempre acontece de um jeito, a rua que a personagem sempre atravessa, a frase que repete antes de algo importante. O ritual revela o que uma personagem ou cultura considera sagrado.",
    "celebracao":    "A celebração na ficção é um dos momentos mais difíceis de escrever sem clichê — a cena de festa, de vitória, de alegria coletiva. A celebração literária mais interessante tem sempre uma camada de tensão ou melancolia: a personagem que sorri mas pensa em quem não está, a festa que marca o fim de algo mesmo sendo início de outra coisa. A alegria pura é difícil de sustentar por muitas páginas; a alegria complexa é humana.",
    "festa":         "A festa na ficção é espaço de revelação: o que as personagens fazem quando estão sob efeito de celebração, álcool, música, desinibição coletiva. A cena de festa é um dos recursos mais antigos da narrativa para revelar caráter. Quem dança com quem, quem fica no canto, quem vai embora cedo, quem não consegue parar — tudo isso diz algo sobre quem é cada personagem quando a guardia baixa.",
    "velorio":       "O velório na ficção é o rito que reúne quem não se reuniria de outra forma — e por isso é espaço narrativo fértil. A morte de alguém convoca presências, resolve pendências não ditas, revela relações que estavam escondidas. O velório literário é frequentemente onde conflitos que vinham se acumulando por gerações chegam à superfície. A morte como catalisador é um dos recursos mais antigos e mais verdadeiros da ficção.",
    "casamento":     "O casamento na ficção é um dos ritos de passagem mais carregados — e por isso aparece tanto como desfecho (ficção do século XIX) quanto como começo de conflito (ficção contemporânea). O casamento literário revela o que a cultura de origem trata como completo: quem se completa com quem, em que condições, com que expectativas. A ficção que subverte ou questiona o casamento está frequentemente questionando a estrutura social que ele representa.",
    "separacao":     "A separação na ficção é o corte que permanece — o fim de uma relação, de uma família, de um pertencimento. A separação literária raramente é limpa: deixa rastros, ressentimentos, reviravoltas, arrependimentos. A separação de personagens que se amam é diferente da separação de personagens que se odeiam; a mais interessante muitas vezes é a separação de personagens que não sabem exatamente o que sentem uma pela outra.",
    "partida":       "A partida na ficção é o movimento de saída — de um lugar, de uma relação, de uma versão de si mesmo. A partida abre a história tanto quanto a fecha: o romance de formação começa com uma partida; o romance nostálgico termina contemplando a que nunca aconteceu. A partida literária revela o que uma personagem deixa para trás e o que carrega sem perceber. O que não cabe na mala é frequentemente o que vai junto.",
    "chegada":       "A chegada na ficção é o encontro com o novo — o lugar que ainda não é familiar, a presença que ainda não tem nome, o começo de um tempo diferente. A chegada literária é frequentemente deslocante: a personagem que chega não sabe as regras, não conhece os rostos, não entende os silêncios. Essa desorientação é dramaticamente rica — a escritora pode revelar o mundo pelo olhar de quem o vê pela primeira vez.",

    // Força interna, corpo e presença — v941
    "insurgencia":   "A insurgência na ficção é o ato de se levantar contra — uma personagem, um povo, uma voz narrativa que recusa a posição que lhe foi dada. A ficção insurgente não precisa ser didática; pode ser sutil, poética, oblíqua. Mas ela tem uma posição. A literatura afro-brasileira e indígena contemporânea pratica a insurgência como forma literária — não apenas como tema, mas como modo de narrar, de escolher quem fala, quem cala, quem sobrevive na história.",
    "sensorialidade":"A sensorialidade na ficção é a presença dos cinco sentidos no texto — o que a personagem toca, cheira, ouve, prova, vê. A ficção sensorial não é decoro: é precisão. Um cheiro específico ancora uma cena no concreto; uma textura diz algo sobre quem está tocando. A escritora que trabalha a sensorialidade cria experiência de leitura, não apenas informação. O leitor não só entende — ele sente.",
    "tato":          "O tato na ficção é o sentido mais íntimo — e por isso o mais difícil de escrever sem cair no clichê. Tocar alguém é diferente de ver alguém; a presença do tato em uma cena muda a distância entre personagens. A ficção que narra o tato com precisão — a temperatura de uma mão, a textura de um tecido, o peso de um abraço — cria intimidade que a descrição visual não alcança. O tato é o sentido que exige mais proximidade.",
    "presenca":      "A presença na ficção é a qualidade de uma personagem ou cena que se impõe — que ocupa espaço, que a leitora sente mesmo quando não está sendo descrita. A presença é efeito de escrita, não de caracterização: uma personagem pode ser descrita em páginas e ser vaga; outra aparece em três linhas e fica. A presença se constrói por especificidade, por gesto único, por voz distinta. Ausência de clichê é o começo da presença.",
    "impulso":       "O impulso na ficção é o que move uma personagem antes que ela tenha tempo de pensar — a reação visceral, o gesto não calculado, a fala que saiu antes de ser formada. O impulso revela o que a personagem é por baixo do que ela quer ser. A ficção que só mostra personagens calculadas e controladas perde uma dimensão: a humanidade está também no impulso, no que não foi planejado, no que a personagem faria de novo mesmo sabendo que não devia.",
    "impeto":        "O ímpeto na ficção é a força que empurra — a personagem que não consegue parar, que age movida por algo maior do que a razão. O ímpeto é primo do impulso, mas tem duração: enquanto o impulso é momentâneo, o ímpeto é uma corrente que atravessa páginas. A ficção de ímpeto tem ritmo próprio: frases mais curtas, menos hesitação, mais ação. O ímpeto pode ser heroico ou destruidor — frequentemente os dois.",
    "necessidade":   "A necessidade na ficção é o que uma personagem não pode não querer — o que está além da preferência, além da escolha razoável, além do que seria prudente. A necessidade dramática é o motor das histórias mais antigas: Édipo precisava saber, Antígona precisava enterrar o irmão, Capitu precisava — de quê, exatamente, é a questão. A necessidade que uma personagem não reconhece em si mesma é mais perigosa do que a que ela admite.",
    "carencia":      "A carência na ficção é a falta que molda — a personagem que age a partir do que não teve, do que perdeu, do que nunca vai ter. A carência literária não é fraqueza de personagem: é motor. A ficção que entende a carência de suas personagens entende de onde vêm as decisões que não fazem sentido de fora. A carência não precisa ser nomeada pela personagem — pode ser visível apenas para a leitora, o que cria uma distância dramática específica.",
    "intencao":      "A intenção na ficção é o que uma personagem quer alcançar — mas a intenção declarada e a intenção real nem sempre são a mesma coisa. Uma personagem pode declarar que quer ajudar e na verdade querer controlar. A intenção oculta é um dos recursos mais eficazes da ficção: a leitora vê o que a personagem não vê em si mesma. Quando intenção e ação divergem, nasce o subtexto — e o subtexto é onde a ficção fica interessante.",
    "proposito":     "O propósito na ficção é a razão de existir de uma personagem — o que dá sentido à sua trajetória dentro da história. O propósito não precisa ser nobre: pode ser banal, destrutivo, equivocado. Mas uma personagem sem nenhum propósito — que reage apenas ao que acontece, sem nenhum vetor próprio — tende a ser passiva e difícil de acompanhar. O arco de personagem frequentemente é a descoberta, a perda ou a transformação do propósito.",

    // Relações humanas, estados internos e crença — v940
    "dependencia":   "A dependência na ficção é a relação em que uma personagem não funciona sem a outra — e isso pode ser amor, pode ser trauma, pode ser sobrevivência. A dependência literária bem construída não é fraqueza: é complexidade. A personagem que sabe que depende e escolhe continuar dependendo é tão interessante quanto a que tenta se libertar. A dependência revela o quanto a autonomia é construção, não dado.",
    "autonomia":     "A autonomia na ficção é a capacidade de uma personagem agir por conta própria — de ter vontade que não é reflexo de outra vontade. A autonomia plena é rara e frequentemente ilusória: mesmo a personagem mais independente é moldada por forças que não escolheu. A ficção que testa a autonomia de uma personagem está testando o quanto o livre-arbítrio é real dentro daquele mundo narrativo.",
    "exclusao":      "A exclusão na ficção é o ato de deixar alguém de fora — do grupo, da comunidade, do pertencimento. A exclusão pode ser declarada ou silenciosa: a personagem que nunca é convidada, a que não entende as piadas, a que percebe pela primeira vez que não pertence. A literatura brasileira tem uma longa tradição de narrar a exclusão por classe, raça, gênero, região — e a escrita dessa exclusão é em si um ato político.",
    "rejeicao":      "A rejeição na ficção é a recusa ativa de pertencimento — quando alguém é expulso, negado, devolvido. A rejeição diferencia-se da exclusão pelo agente: a exclusão pode ser sistêmica; a rejeição tem rosto. A personagem rejeitada carrega uma ferida específica: não apenas a dor do fora, mas a memória de quem disse não. A ficção que narra a rejeição precisa cuidar do ponto de vista: narrar de dentro é diferente de narrar de fora.",
    "aceitacao":     "A aceitação na ficção tem dois sentidos que a escritora precisa distinguir: a aceitação-resignação (deixar de lutar) e a aceitação-integração (incorporar o que é sem negar). A personagem que aceita a morte de alguém querido, que aceita sua própria condição, que aceita o outro como é — essa aceitação pode ser o maior arco de uma história. A ficção contemporânea desconfia da aceitação fácil: exige que ela seja conquistada, não apenas declarada.",
    "reconhecimento": "O reconhecimento na ficção é o momento em que uma personagem é vista pelo que é — não pelo que aparenta, não pelo papel que ocupa, mas pelo que de fato é. Axel Honneth desenvolveu uma teoria do reconhecimento como necessidade humana fundamental. A ficção que narra a luta por reconhecimento narra uma das histórias mais antigas: quero ser visto. O não-reconhecimento — a invisibilidade imposta — é uma das formas de violência mais difíceis de nomear.",
    "choro":         "O choro na ficção é difícil de escrever sem cair no sentimentalismo — mas é um dos gestos mais humanos e mais presentes na vida real. A escritora que evita o choro por medo do melodrama muitas vezes perde uma cena verdadeira. O choro literário funciona quando está contextualizado: quando sabemos o que está sendo chorado, quando o choro tem causa específica, quando a personagem não chora em vez de toda a ficção chorar junto.",
    "crenca":        "A crença na ficção é o que uma personagem aceita como verdadeiro sem poder provar — e que orienta suas ações. A crença religiosa é a mais óbvia, mas há crença em pessoas, em sistemas, em histórias que nos contamos sobre nós mesmos. A ficção que examina uma crença não precisa julgá-la: pode simplesmente mostrar o que ela sustenta, o que ela exige, o que acontece quando ela entra em colapso.",
    "duvida":        "A dúvida na ficção é o motor de muitas histórias — a personagem que não sabe se pode confiar, se fez certo, se o que viu foi real. A dúvida literária bem sustentada cria tensão sem precisar de ação: apenas de uma personagem que carrega uma pergunta sem resposta. A narradora não-confiável é construída sobre a dúvida que instila no leitor. A dúvida sobre o passado, sobre o outro, sobre si mesma são três formas diferentes de narrativa.",
    "certeza":       "A certeza na ficção é perigosa — não porque seja errada, mas porque a personagem certa demais perde complexidade. A ficção moderna desconfia da certeza: a personagem que sabe com absoluta segurança o que é certo frequentemente está prestes a aprender que não sabia. A certeza que se mantém até o fim é rara e exige fundação muito sólida. A certeza como ponto de chegada — conquistada após dúvida — é uma das resoluções mais satisfatórias da ficção.",

    // Narratologia avançada, voz e ruptura — v939
    "lexico":          "O léxico é o conjunto de palavras de uma língua — ou, para a escritora, o conjunto de palavras que ela usa. O léxico de um texto revela quem o escreveu: vocabulário de qual classe social, de qual região, de qual época, de qual leitura. A ampliação do léxico não é acúmulo de palavras raras; é o aumento das possibilidades de precisão — encontrar a palavra certa em vez de usar a aproximada.",
    "sintaxe":         "A sintaxe é a organização das palavras na frase — mas para a escritora, a sintaxe é o ritmo visível. Uma frase longa com muitas subordinadas cria uma sensação diferente de uma sequência de frases curtas. A sintaxe de Clarice Lispector é inconfundível não porque use palavras raras, mas porque a ordem das palavras quebra expectativas. Mudar a sintaxe é mudar o tempo da leitura.",
    "eufonia":         "A eufonia é a harmonia sonora de um texto quando lido em voz alta — o oposto da cacofonia. A prosa eufônica não é necessariamente suave: pode ser áspera de propósito, mas a aspereza está controlada. A eufonia importa mesmo para textos que ninguém vai ler em voz alta, porque o leitor interno ainda ouve. Ler o próprio texto em voz alta é o teste mais direto de eufonia: o que tropeça, tropeça.",
    "hipodiegese":     "A hipodiégese é a narrativa dentro da narrativa — a história que uma personagem conta dentro da história principal. As Mil e Uma Noites é o exemplo extremo: Sheherazade conta histórias em que personagens contam histórias. A hipodiégese cria distância controlada: a escritora pode narrar algo perturbador como história que uma personagem conta, suavizando ou intensificando o efeito conforme a moldura.",
    "correcao":        "A correção na escrita é a etapa em que a escritora revê o texto não para melhorá-lo, mas para removê-lo do erro — erros de ortografia, concordância, pontuação, consistência interna. A correção é diferente da revisão: a revisão pode mudar a estrutura; a correção apenas garante que o que estava escrito está escrito corretamente. A confusão entre as duas faz escritoras correrem muito cedo para o pequeno e tarde demais para o grande.",
    "omissao":         "A omissão na ficção é o que a escritora decidiu não dizer — e que por isso continua presente no texto como ausência significativa. Hemingway formulou a teoria do iceberg: o que está fora da página sustenta o que está na página. A omissão bem-feita não é buraco: é peso implícito, o que a leitora sente sem conseguir nomear. Saber o que omitir é tão importante quanto saber o que escrever.",
    "fratura":         "A fratura no texto é a quebra deliberada de continuidade — de tempo, de voz, de perspectiva, de tom. A ficção moderna usa a fratura como recurso: um flashback não anunciado, uma mudança de pessoa no meio do capítulo, uma voz que entra sem apresentação. A fratura bem executada cria desconforto produtivo; a fratura acidental cria confusão. A diferença está na consistência do desconforto.",
    "descontinuidade":  "A descontinuidade é a qualidade de um texto que não avança linearmente — que salta, retorna, omite, fragmenta. A descontinuidade pode ser estrutural (capítulos fora de ordem, seções sem numeração, narrativas paralelas que nunca se tocam) ou estilística (frases incompletas, parágrafos que começam no meio de um pensamento). Autoras como Clarice Lispector e Hilda Hilst fizeram da descontinuidade uma assinatura.",
    "interrupcao":     "A interrupção na ficção é o corte — a frase que não termina, o diálogo que cessa antes da resposta, o capítulo que para no meio da ação. A interrupção forçada pela forma (em vez de pela história) produz efeitos específicos: urgência, suspense, desconforto. O capítulo que termina no momento errado obriga a leitora a virar a página. A interrupção é uma das ferramentas mais diretas de controle de ritmo.",
    "fragmento":       "O fragmento é a unidade mínima de um texto que se organiza por acumulação em vez de desenvolvimento linear. A prosa fragmentada não conta uma história que avança: reúne pedaços que formam sentido pelo conjunto. A poesia em prosa e o diário fragmentado são formas naturais do fragmento. No romance, o fragmento pode aparecer como capítulos curtíssimos, insertos de outros textos, anotações avulsas. O risco do fragmento é a dispersão; o poder é a intensidade concentrada.",

    // Condição humana e qualidades textuais — v938
    "testemunha":    "A testemunha na ficção é quem viu — e cuja visão transforma o texto. A ficção de testemunho tem peso diferente da ficção puramente imaginada: não porque seja mais verdadeira, mas porque a responsabilidade de quem testemunha é diferente. A escritora que testemunha algo e transforma em ficção está fazendo uma escolha ética sobre o que vai guardar, o que vai mudar, e o que deve o leitor saber sobre a origem do texto.",
    "sobrevivente":  "O sobrevivente na ficção é a personagem que atravessou o que poderia tê-la destruído — e que carrega essa travessia no corpo, na memória, na forma de estar no mundo. A ficção de sobrevivência não é só sobre o que aconteceu: é sobre como se vive depois. O sobrevivente literário raramente é heróico no sentido clássico; é humano no sentido mais completo — marcado, fraturado, mas ainda aqui.",
    "migrante":      "O migrante na ficção é a personagem entre dois mundos — que saiu de um lugar e chegou em outro, mas que nunca pertence completamente a nenhum dos dois. A ficção migrante brasileira tem camadas: o migrante interno (nordestino no sudeste, rural na cidade) e o migrante internacional (o brasileiro no exterior, o estrangeiro no Brasil). O migrante literário carrega uma sensação de deslocamento que a ficção sedentária não conhece.",
    "ancestral":     "O ancestral na ficção é quem veio antes e cuja presença ainda molda o presente. O ancestral literário não é necessariamente uma pessoa específica: pode ser o que veio de um povo, de uma comunidade, de uma terra. A ficção afro-brasileira e indígena trabalha com a ancestralidade como força ativa — não o passado que passou, mas o passado que continua operando no corpo, na memória e na identidade de quem vive agora.",
    "descendente":   "O descendente na ficção é quem herda — o que foi transmitido sem escolha, o que chega antes de qualquer decisão. A condição de descendente carrega tanto peso quanto privilégio, dependendo da herança: descendente de escravizados, de colonizadores, de migrantes, de revolucionários. A ficção que examina a descendência pergunta o que fazemos com o que recebemos sem ter pedido.",
    "herdeiro":      "O herdeiro na ficção é o descendente que vai receber — ou que já recebeu — a herança material ou simbólica de quem veio antes. O herdeiro do romance realista é frequentemente alguém que recebe mais do que merece e precisa aprender a merecê-lo — ou que recebe e desperdiça, ou que recusa. A herança na ficção raramente é simples: é peso, responsabilidade, contradição.",
    "vazio":         "O vazio na ficção é a ausência que tem presença — o que falta e que organiza tudo ao redor. O vazio da personagem que perdeu alguém, o vazio do lugar que foi destruído, o vazio entre o que a personagem queria ser e o que se tornou. A ficção que trabalha com o vazio não precisa preenchê-lo: pode deixá-lo como lacuna estrutural, como buraco que a leitora reconhece porque também tem o seu.",
    "perdao":        "O perdão na ficção é um dos movimentos mais difíceis de narrar — porque exige que a personagem faça algo que não pode ser desfeito e que não pode ser explicado simplesmente. O perdão literário não é esquecimento nem resolução: é a decisão de não deixar a ferida definir o futuro. A ficção que usa o perdão como desfecho fácil trai a complexidade do que está narrando; a que o deixa difícil, incerto, talvez impossível, honra o que é.",
    "complexidade":  "A complexidade na ficção é a qualidade de ter mais de uma camada — de não poder ser resumida sem perda. A ficção complexa não é difícil por capricho: é densa porque o que está sendo narrado exige densidade. A complexidade literária bem trabalhada não afasta a leitora: aprofunda a experiência de leitura a cada retorno ao texto. O que parece simples na primeira leitura e revela mais na segunda é mais complexo do que parece.",
    "profundidade":  "A profundidade na ficção é a extensão do mergulho que o texto permite — o quanto a leitora pode ir além da superfície. Um texto com profundidade não entrega tudo na primeira leitura; guarda algo para quem volta, para quem lê com atenção diferente, para quem cresceu entre uma leitura e outra. A profundidade não é resultado de hermetismo: é resultado de ter construído algo com raízes, não apenas superfície.",
    "leveza":        "A leveza na ficção é a qualidade que Calvino colocou como primeira das suas 'Seis propostas para o próximo milênio' — não a ausência de peso, mas a capacidade de mover-se com agilidade mesmo carregando. A leveza literária não é superficialidade: é a prosa que trata de coisas graves sem esmagar a leitora, que encontra o ângulo de entrada que permite respirar mesmo no peso. A leveza é conquista técnica, não ausência de ambição.",
    "densidade":     "A densidade na ficção é o peso informacional e emocional por página — o quanto cada parágrafo carrega. A ficção densa não é necessariamente lenta: pode mover-se com velocidade mas com cada frase grávida de sentido. O risco da densidade excessiva é a opacidade; o risco da leveza excessiva é a vacuidade. A ficção que encontra a densidade certa para o que quer dizer encontrou sua forma.",

    // Autoria, oralidade e escrita íntima — v934
    "pseudonimo":    "O pseudônimo é o nome que a escritora usa no lugar do nome civil — uma identidade de autoria construída. As razões para usar pseudônimo são muitas: proteger a vida privada, escapar de preconceito de gênero, distinguir produções de tipos diferentes, começar de novo. George Eliot, George Sand, Clarice Lispector (que publicou Perto do Coração Selvagem sem revelar ser imigrante recente) — o pseudônimo é parte da história de quem pode escrever e se revelar.",
    "autoria":       "A autoria é a relação entre uma pessoa e um texto que ela criou — mas essa relação é mais complexa do que parece. Barthes proclamou a morte do autor: o texto existe independente de quem o escreveu, e a intenção da autora não governa o sentido. Foucault perguntou o que é um autor: uma função discursiva, não uma pessoa. No Brasil contemporâneo, a autoria ganha dimensão política: quem tem o direito de assinar o quê, quem pode falar de quê.",
    "pacto":         "O pacto na ficção é o acordo implícito entre texto e leitora sobre o que está sendo oferecido. O pacto autobiográfico (Philippe Lejeune) é o que estabelece que o 'eu' do texto é a mesma pessoa que a autora do livro e a narradora. O pacto ficcional é o oposto: a leitora aceita que o que está sendo narrado não é verdadeiro mas importa como se fosse. Quebrar o pacto sem aviso é trair a leitora; quebrar com consciência é metaficção.",
    "quiasmo":       "O quiasmo é a figura de linguagem que inverte a ordem de elementos equivalentes: 'viver para escrever' / 'escrever para viver'. O quiasmo cria simetria e contraste ao mesmo tempo — a mesma relação, mas com os termos trocados. Na prosa literária, o quiasmo pode aparecer em escala de frase ou de parágrafo: uma situação narrada que espelha e inverte outra. O quiasmo bem usado cria a sensação de que o texto dobrou sobre si mesmo.",
    "musicalidade":  "A musicalidade na prosa é a qualidade sonora da escrita — o ritmo, a alternância de sons, a melodia que emerge quando o texto é lido em voz alta. A musicalidade não é ornamento: é parte do sentido. Uma frase com muitas sílabas tônicas seguidas cria tensão; uma frase mais suave, com vogais abertas, cria distensão. A escritora que não pensa no som de suas frases está perdendo uma dimensão do texto.",
    "sonoridade":    "A sonoridade é a qualidade acústica das palavras — o que elas soam, como interagem entre si quando postas lado a lado. A aliteração, a assonância, a cacofonia, a eufonia: todas são qualidades de sonoridade. A ficção que cuida da sonoridade sem fazer da sonoridade o tema cria uma camada de prazer sensorial que a leitora sente antes de analisar. O texto sonoramente agradável é mais fácil de habitar.",
    "depoimento":    "O depoimento na ficção é o relato em primeira pessoa de uma experiência vivida — com marcas de oralidade, de urgência, de testemunho direto. O depoimento literário não é transcrição: é construção que preserva a voz de quem viveu. A literatura de testemunho, o relato de sobreviventes, a literatura carcerária: formas que usam o depoimento como estrutura e como ética, porque a vida que está sendo relatada exige um modo de narrar à sua altura.",
    "relato":        "O relato na ficção é o modo narrativo que conta o que aconteceu — mais descritivo que dramático, mais informativo que evocativo. O relato resume onde a cena mostra; o relato reconta onde o diálogo atualiza. O relato é recurso de velocidade: passa por períodos da história que a narrativa não precisa encenar. A dosagem entre relato e cena é uma das decisões estruturais mais importantes que a escritora toma.",
    "memorialismo":  "O memorialismo é a escrita que reconstrói o passado a partir da memória — não como autobiografia sistemática, mas como fragmentos, imagens, afetos. O memorialismo literário brasileiro tem exemplos fundamentais: Pedro Nava, Graciliano Ramos, Carolina Maria de Jesus. O memorialismo reconhece que a memória não é arquivo: é interpretação. O que fica não é o que aconteceu, mas o que ficou, e por quê ficou.",
    "glossario":     "O glossário no livro literário é a lista de termos que precisam de explicação — vocabulário regional, jargão de época, línguas indígenas ou africanas, neologismos da autora. O glossário é uma decisão estética: colocar explicações no próprio texto atrapalha o fluxo; colocá-las no glossário preserva o ritmo mas pode interromper a leitura para quem consulta. A autora que precisa de glossário está escrevendo para leitoras que não conhecem seu mundo — e está decidindo se vai ou não facilitar a travessia.",
    "confissao":     "A confissão na ficção é o texto que revela o que foi ocultado — que expõe o que a narradora ou a personagem normalmente guardaria. A confissão literária tem longa história: Agostinho, Rousseau, Kafka. A confissão contemporânea brasileira, especialmente de escritoras negras e periféricas, é também confissão política: revelar o que foi vivido é nomear o que o sistema preferia que ficasse sem nome.",
    "reminiscencia": "A reminiscência é a memória que vem sem ser chamada — o fragmento do passado que surge no presente a partir de um cheiro, um som, uma palavra. A reminiscência literária é diferente da memória intencional: ela chega com a intensidade do involuntário, com a nitidez das imagens que o tempo guardou com cuidado especial. A ficção que usa reminiscências cria um tempo não linear que é fiel à forma como a memória realmente funciona.",

    // Cultura oral, editorial e vocabulário ético — v933
    "slam":          "O slam de poesia é a competição de poesia falada — onde as autoras recitam seus textos para uma plateia que julga, com aplausos, gritos e notas. O slam no Brasil tem raízes nos saraus da periferia paulistana e nas batalhas de rima do hip-hop. O slam é ao mesmo tempo forma literária, ritual de comunidade e escola de voz: a escritora que aprende slam aprende que a poesia existe antes e depois da página.",
    "sarau":         "O sarau é o evento literário que reúne escritoras, leitoras e público para compartilhar textos — em voz alta, com presença. O sarau da periferia brasileira nas décadas de 2000 e 2010 foi um dos movimentos literários mais importantes do país: uma forma de produção e circulação de literatura que não precisava de editora, de livraria ou de legitimação da crítica cultural hegemônica.",
    "argumento":     "O argumento literário é o resumo mais desenvolvido que a sinopse — não apenas o que acontece, mas como e por quê. O argumento revela a estrutura: os atos, as viradas, o arco dos personagens. Para projetos de roteiro e para propostas editoriais de ficção longa, o argumento é o documento que mostra se a autora sabe para onde vai a história antes de terminá-la.",
    "proposta":      "A proposta editorial é o documento que a escritora envia para agentes ou editoras antes de submeter o manuscrito completo. Para ficção, a proposta geralmente inclui sinopse, primeiras páginas e informações sobre a autora. Para não-ficção, inclui mais: sumário, amostra, público-alvo, comparáveis de mercado. A proposta é o texto que vende o livro antes que o livro exista em sua forma completa.",
    "parcial":       "Na submissão editorial, o parcial é o conjunto de primeiras páginas ou capítulos que a escritora envia junto com a sinopse — em geral as primeiras 50 ou 100 páginas. O pedido de parcial por parte de um agente ou editora é sinal de interesse: significa que a sinopse convenceu o suficiente para ver como a escrita funciona. Do parcial pode vir o pedido de manuscrito completo.",
    "serie":         "A série na ficção é a narrativa que se estende por múltiplos volumes com continuidade de personagens, mundo ou arco narrativo. A série exige mais do que um livro bom: exige um mundo grande o suficiente para sustentar muitos livros, e personagens com capacidade de se desenvolver ao longo de volumes. A série que decai depois do primeiro volume frequentemente revela que a ideia era de um livro, não de muitos.",
    "trilogia":      "A trilogia é a forma mais clássica da ficção seriada: três volumes que formam uma unidade. A trilogia tem estrutura implícita — estabelecimento, complicação, resolução — que pode ou não corresponder à divisão dos volumes. A trilogia que funciona é aquela em que cada volume tem sua própria forma completa e ao mesmo tempo contribui para o arco maior; a que falha é a que usa o segundo livro só para adiar o terceiro.",
    "duologia":      "A duologia é a narrativa em dois volumes — uma forma que ganhou presença especialmente na ficção jovem adulta. A duologia tem desafio particular: dois volumes precisam sustentar um mundo sem o benefício do terceiro livro para resolver questões abertas. A divisão tem que ser dramática, não apenas cronológica: o fim do primeiro volume tem que ser significativo por si mesmo, não apenas uma interrupção.",
    "escrevivencia": "Escrevivência é o conceito da escritora brasileira Conceição Evaristo: a escrita que nasce da experiência vivida, que não separa a vida da escrita. A escrevivência é a prática de escrever a partir de dentro — do corpo, da memória, da história coletiva. O termo nomeia o que muitas escritoras negras brasileiras fazem: transformar a vida, inclusive o sofrimento e a resistência, em escrita que não é autobiografia mas não é ficção pura.",
    "acaso":         "O acaso na ficção é o elemento que não foi causado por nenhuma personagem — o encontro inesperado, o acidente, a coincidência. O acaso literário tem má fama porque pode parecer deus ex machina: solução que vem de fora. Mas o acaso é parte da vida, e a ficção que o ignora completamente também distorce. O acaso bem usado é o que muda as circunstâncias e revela como as personagens respondem ao que não controlaram.",
    "consequencia":  "A consequência na ficção é o que acontece depois — o efeito que as escolhas das personagens produzem no mundo da história. A ficção sem consequência é ficção sem peso: se as personagens podem fazer qualquer coisa sem que o mundo responda, nada importa. A consequência é o que cria causalidade: esta cena acontece porque aquela aconteceu. E a leitora que percebe essa cadeia está dentro da narrativa.",
    "responsabilidade": "A responsabilidade na ficção é tanto ética quanto narrativa. Ética: a escritora que narra experiências de grupos marginalizados tem responsabilidade com como os representa. Narrativa: a personagem que age tem responsabilidade pelas consequências — mesmo que não as preveja, mesmo que não as queira. A ficção que evita a responsabilidade de suas personagens as infantiliza; a que as força a responder pelo que fizeram as humaniza.",

    // Perspectiva, ficção e tempo — v932
    "instantaneo":   "O instante na ficção é o tempo que a narrativa pode expandir indefinidamente — o momento que dura páginas enquanto o relógio mal avança. O romance pode esticar um segundo até ele conter toda a vida de uma personagem, ou comprimir anos em uma linha. O instantâneo que a ficção escolhe expandir é onde ela declara que o tempo importa.",
    "eternidade":    "A eternidade na ficção é o tempo que escapa da história — o que existe antes do começo e depois do fim da narrativa. A ficção que toca a eternidade frequentemente trabalha com personagens que sentem a passagem do tempo de forma diferente: quem viveu muito, quem vai morrer, quem ama alguém que vai desaparecer. A eternidade literária não é abstração religiosa: é a consciência de que o tempo de uma vida é finito.",
    "suspensao":     "A suspensão na ficção é o estado em que a leitora sabe que algo vai acontecer mas ainda não sabe o quê — ou sabe o quê mas não o quando. A suspensão sustentada é o motor do thriller e do horror, mas opera em toda boa ficção: o que vai acontecer com essa personagem de quem me importo? A suspensão que se resolve rápido demais esgota a tensão; a que se prolonga sem recompensa frustra.",
    "ilusao":        "A ilusão na ficção é o que o texto faz a leitora acreditar — que o mundo narrado existe, que as personagens têm vida, que o que está sendo lido importa. A ilusão literária é frágil: qualquer erro de consistência, qualquer frase que lembra que existe uma autora do outro lado, pode quebrá-la. A ficção metaficcional joga com essa fragilidade; a ficção convencional a cuida como condição de funcionamento.",
    "realidade":     "A realidade na ficção é o que o mundo narrado apresenta como verdadeiro dentro de si mesmo — não a realidade externa, mas a realidade interna da história. O que é real na ficção realista tem regras diferentes do que é real na ficção fantástica. A questão 'isso poderia acontecer de verdade?' é a pergunta errada: a questão é se isso poderia acontecer nesse mundo, com essas regras, com essas personagens.",
    "subjetividade": "A subjetividade na ficção é a perspectiva marcada por quem vê, sente e narra — o oposto da pretensão de neutralidade. A ficção subjetiva não esconde que está sendo contada por alguém com história, com corpo, com posição no mundo. A subjetividade bem trabalhada não é fraqueza: é honestidade sobre de onde a narrativa fala. A ficção que finge não ter ponto de vista geralmente tem um ponto de vista muito específico que prefere não declarar.",
    "objetividade":  "A objetividade na ficção é a ilusão de que a narrativa mostra sem julgar — que descreve sem filtro. A objetividade total é impossível: toda escolha narrativa já é julgamento. Mas a ficção pode criar efeito de objetividade: narração que recusa explicar, que apresenta os fatos e deixa a leitora concluir. O efeito de objetividade tem poder — a cena que a narradora descreve sem comentário e que a leitora julga por conta própria.",
    "relatividade":  "A relatividade na ficção é o reconhecimento de que a verdade do texto depende de quem narra. O mesmo evento contado por personagens diferentes produz histórias diferentes — todas verdadeiras para quem as conta. A ficção que trabalha com perspectivas múltiplas sobre o mesmo evento está mostrando que a realidade é relativa à posição de quem olha. Isso não é niilismo: é complexidade narrativa.",
    "parcialidade":  "A parcialidade na ficção é o que a narradora escolhe mostrar e o que escolhe ocultar — consciente ou inconscientemente. Toda narração é parcial: nenhuma narradora tem acesso a tudo, e mesmo que tivesse, não poderia contar tudo. A parcialidade que a ficção reconhece como tal — a narradora que sabe que não sabe tudo — é mais honesta que a que finge ser completa. A parcialidade não revelada é manipulação; a revelada é arte.",
    "neutralidade":  "A neutralidade na ficção é uma posição impossível que algumas narrativas simulam. O texto que parece não tomar partido sobre o que narra frequentemente está tomando o partido mais conservador: o de que tudo está bem como está, de que as coisas são como são e isso não precisa ser questionado. A neutralidade declarada é sempre suspeita; a neutralidade eficaz é aquela que a leitora não percebe como escolha.",

    // Qualidade, mundo e dramaturgia — v931
    "universo":      "O universo da ficção é o conjunto coerente de regras, lugares, personagens e história que tornam um mundo possível. O universo de Tolkien, o universo de Ursula Le Guin, o universo de Gaiman: cada um tem suas próprias leis, que a ficção mantém com consistência. A leitora aceita qualquer universo que a narrativa construa com cuidado; o que não aceita é o universo que quebra suas próprias regras sem razão.",
    "cosmologia":    "A cosmologia na ficção é a estrutura mais ampla do universo narrado — como o mundo foi criado, como é organizado, quais são as forças fundamentais que o governam. A cosmologia literária não precisa ser explícita: pode estar nas pequenas consistências do mundo, na forma como o sobrenatural funciona, no que os personagens acreditam sobre a origem das coisas. O que a cosmologia revela é o que o autor acredita que está por baixo de tudo.",
    "mitologia":     "A mitologia na ficção é o conjunto de histórias fundadoras que explicam o mundo dos personagens — o que aconteceu no começo, quem são os seres maiores que os humanos, o que os antepassados fizeram. A mitologia literária pode ser histórica (a mitologia grega ou tupi como matéria) ou criada (a mitologia inventada de uma saga). A ficção que constrói mitologia própria está criando um sistema de significado, não apenas decoração.",
    "didascalia":    "A didascália é a indicação cênica no texto dramático — as rubricas que dizem onde a cena acontece, como as personagens se movem, o que está no espaço. Na leitura dramática, a didascália é visível; na encenação, ela desaparece na concretização. A didascália bem escrita é sóbria: diz o que é necessário para o sentido da cena sem transformar o texto dramático em roteiro de cinema.",
    "replica":       "A réplica no diálogo dramático é a fala de uma personagem em resposta à fala de outra — o turno de fala que mantém o jogo em andamento. A réplica bem escrita faz mais de uma coisa ao mesmo tempo: responde ao que foi dito, revela a personagem, avança a cena. A réplica que apenas responde ao que foi perguntado é funcional mas plana; a que responde e desviar é mais viva.",
    "consistencia":  "A consistência na ficção é a fidelidade às regras que o próprio texto estabeleceu — as personagens agem como quem são, o mundo funciona como foi apresentado, a voz narrativa mantém seu nível. A inconsistência quebra o pacto com a leitora: quando um personagem age de forma contrária a quem estava sendo, sem razão narrativa, a leitora percebe que a autora o estava usando, não narrando.",
    "coerencia":     "A coerência na ficção é a unidade profunda do texto — a sensação de que tudo faz parte do mesmo projeto, que as partes respondem umas às outras. A coerência não é uniformidade: uma narrativa pode ter vozes muito diferentes e ainda ser coerente, desde que a variação sirva a um propósito. A incoerência que a leitora sente é quando elementos do texto parecem pertencer a histórias diferentes.",
    "coesao":        "A coesão textual é o tecido que une as frases e parágrafos — os pronomes que retomam substantivos, os conectivos que mostram relação entre ideias, a progressão temática que faz o texto avançar sem saltos. A coesão pobre faz o texto parecer uma lista de frases independentes; a coesão excessiva, mecânica, faz o texto parecer uma demonstração de gramática. A coesão literária é a que a leitora não percebe porque está a serviço do sentido.",
    "clareza":       "A clareza na ficção não é simplicidade — é a qualidade de ser exatamente tão claro quanto a intenção exige. O texto hermético que não pretende ser compreendido imediatamente pode ser claro nos seus termos. O texto que pretende comunicar e não consegue não é misterioso: é falho. A clareza literária é precisão de intenção: a autora sabe o que quer que a leitora receba, e o texto entrega isso.",
    "precisao":      "A precisão na ficção é a escolha da palavra exata — não a que é suficiente, mas a que é certa. A precisão literária não é pedantismo: é respeito pela diferença entre palavras que parecem sinônimas mas não são. 'Andou', 'caminhou', 'marchou', 'arrastou' — cada verbo carrega algo diferente. A escritora precisa é a que sabe a diferença e a escolhe com intenção.",
    "revelacao":     "A revelação na ficção é o momento em que a leitora (e às vezes a personagem) descobre algo que muda como entende a história. A revelação bem construída não é surpresa arbitrária: olhando para trás, estava lá desde o começo — mas a narrativa a ocultou sem mentir. A revelação que a leitora 'não viu vir' mas que em retrospecto 'faz todo sentido' é o ideal do gênero.",
    "giro":          "O giro narrativo (ou virada) é o ponto em que a história muda de direção — quando o que parecia ser a narrativa se revela ser outra coisa, ou quando um elemento central é recontextualizado. O giro pode ser de enredo (o assassino era o detetive), de perspectiva (a narradora estava errada sobre o que via) ou de interpretação (o que parecia amor era controle). O giro que funciona é o que surpreende sem trair.",

    // Gêneros, cânone e narratologia — v930
    "dramaturgia":   "A dramaturgia é a arte de escrever para o palco — o texto que existe para ser dito e encenado, não lido em silêncio. A dramaturgia brasileira tem tradições ricas: o teatro épico de Boal, o teatro negro, o teatro de rua, o teatro da periferia. A escritora que vem da dramaturgia para a prosa carrega marcas: diálogos precisos, ação em tempo presente, cenas construídas para serem vistas.",
    "horror":        "O horror na ficção é o gênero que usa o medo como método — não o susto rápido, mas o medo que fica. O horror literário trabalha com o que não pode ser completamente nomeado ou visto: a ameaça que se percebe antes de se entender. No Brasil, o horror tem raízes no sobrenatural afro-brasileiro, nas histórias de assombração regional, e numa tradição oral que a literatura escrita está só começando a organizar.",
    "thriller":      "O thriller é o gênero da tensão sustentada — não o mistério que precisa ser resolvido, mas o perigo que precisa ser sobrevivido. O thriller cria urgência: a personagem sabe que está em risco, a leitora sabe que está em risco, e o tempo narrativo acelera. O thriller psicológico acrescenta a camada de que o perigo pode estar dentro da personagem, não apenas do lado de fora.",
    "policial":      "O romance policial é o gênero que organiza a narrativa em torno de um crime e de sua investigação. Mas o policial brasileiro raramente segue a fórmula clássica inglesa do enigma elegante: o crime no Brasil tem contexto social, o corpo não está numa biblioteca, a investigadora não é um excêntrico gênio. O policial periférico, o policial feminista, o policial afro-brasileiro estão reescrevendo o gênero a partir de dentro.",
    "aventura":      "A aventura na ficção é o gênero do movimento — a personagem que vai de um lugar para outro e enfrenta perigos no caminho. A aventura clássica tem herói, jornada, obstáculos e retorno. Mas a aventura pode ser subvertida: a personagem que foge ao invés de enfrentar, a jornada sem retorno, o herói que descobre que o vilão estava certo. A aventura como forma é ampla o suficiente para conter contradições.",
    "canone":        "O cânone literário é o conjunto de obras consideradas essenciais — o que 'todo mundo deveria ter lido'. O problema é que o cânone não é neutro: é produto de escolhas históricas feitas por críticos, editoras e universidades que tendiam a ser brancos, europeus e homens. O cânone literário brasileiro está sendo reescrito: vozes que nunca foram incluídas estão sendo reconhecidas, e obras que foram esquecidas estão sendo reeditadas.",
    "influencia":    "A influência na ficção é a marca de quem veio antes — visível ou invisível, reconhecida ou negada. Toda escritora tem influências; o trabalho literário é fazer algo com elas. A influência que liberta é a que foi digerida: não imita a superfície, mas absorveu o princípio. A influência que aprisiona é a que se torna modelo sem questionamento. Harold Bloom chamou de 'angústia da influência' o que as escritoras sentem diante dos gigantes que as formaram.",
    "metalepse":     "A metalepse é o cruzamento entre dois níveis narrativos que deveriam permanecer separados — quando a narradora fala diretamente com a personagem, quando a personagem toma consciência de que está num romance, quando o autor entra na ficção. A metalepse é recurso metaficcional: chama atenção para a artificialidade da narrativa ao mesmo tempo que explora o que acontece quando as fronteiras entre mundos escorregam.",
    "extradiegetico": "O narrador extradiegético é o que narra de fora da história — não é personagem do que narra, não existe no mesmo mundo diegético. A maioria das narradoras oniscientes é extradiegética: contam a história de um lugar que está além dela. A distinção entre extradiegético e intradiegético (narrador dentro da história) organiza quem está contando e de que posição.",
    "intradiegetico": "O narrador intradiegético é o que existe dentro da história que conta — é personagem do mundo diegético. A narradora que conta sua própria história em primeira pessoa é intradiegética; a narradora-personagem que conta a história de outro também. A posição intradiegética tem consequências: o que a narradora pode saber, o que pode distorcer, o que pode omitir — tudo depende de onde ela está dentro do mundo que narra.",
    "subversao":     "A subversão na ficção é o movimento que usa as convenções de um gênero para contrariá-las de dentro. A subversão literária não ignora a tradição: a conhece bem o suficiente para desviar dela com precisão. O conto de fadas subvertido, o romance policial que não resolve o crime, a ficção científica que rejeita o progresso — cada subversão produz sentido em relação ao que estava subvertendo.",
    "convencao":     "A convenção na ficção é o acordo implícito entre a narrativa e a leitora sobre as regras do jogo — o que pode acontecer neste tipo de história, como o mundo funciona, o que é permitido esperar. As convenções do gênero, da época, da tradição literária: todas criam expectativas que a ficção pode cumprir ou frustrar. Conhecer as convenções é condição para usá-las com consciência — para cumpri-las ou para subvertê-las.",

    // Sentidos, voz e técnica — v929
    "olfato":        "O olfato é o sentido mais subestimado na ficção — e o mais poderoso para convocar memória. O cheiro que a personagem reconhece sem querer, que a transporta para outro tempo, que a faz sentir o que pensava ter esquecido: o olfato na ficção é acesso direto ao inconsciente da personagem. A ficção que inclui cheiros com precisão cria presença que a descrição visual sozinha não alcança.",
    "paladar":       "O paladar na ficção é memória e identidade ao mesmo tempo. A comida que a personagem conhece desde criança, o sabor que não consegue reproduzir em outro lugar, o que o corpo reconhece antes da mente — o paladar literário é sempre autobiográfico, mesmo em ficção. A cena de comida bem escrita não descreve o prato: descreve o que o prato significa para quem come.",
    "audicao":       "A audição na ficção é o sentido que constrói o mundo fora do campo de visão. O que a personagem ouve sem ver — os passos, a voz no corredor, a música que vem de outro quarto — expande o espaço da cena além do que está sendo descrito. A ficção que trabalha com sons com precisão cria uma camada de mundo que a descrição visual não alcança.",
    "visao":         "A visão é o sentido dominante na ficção convencional — e por isso o mais fácil de usar de forma genérica. A cena que só descreve o que se vê muitas vezes perde o que os outros sentidos carregariam. Quando a visão é usada com precisão — o detalhe específico que só essa personagem notaria, nessa luz, nesse momento — ela cria presença. O clichê visual é o que qualquer personagem veria; o detalhe literário é o que só ela vê.",
    "piedade":       "A piedade na ficção é a emoção que a leitora sente pela personagem — e que a narrativa pode manipular. A personagem que inspira piedade em excesso pode perder complexidade: quando só sentimos pena, deixamos de reconhecer agência. A ficção que equilibra piedade com respeito à personagem cria uma relação mais honesta com a leitora do que a que usa sofrimento para extrair reação emocional.",
    "distancia":     "A distância narrativa é o quanto a narradora se afasta (ou se aproxima) da personagem e da cena. Distância grande: narração mais objetiva, menos acesso ao interior. Distância pequena: narração mais subjetiva, mais próxima do fluxo da personagem. A distância não é fixa no texto: a narradora pode se aproximar e se afastar, e esse movimento cria diferentes efeitos de intimidade e de perspectiva.",
    "intimidade":    "A intimidade narrativa é o que a narração revela que a personagem não revelaria em voz alta. O pensamento que não chegou a ser fala, o sentimento que a personagem ainda não nomeou, o que o corpo fez antes da mente entender — a intimidade literária é acesso ao que está antes da expressão. A ficção que consegue intimidade sem invasão — que revela sem expor — encontra o ponto mais delicado da técnica narrativa.",
    "autoridade":    "A autoridade narrativa é o quanto a leitora confia na narradora — e é construída ou destruída frase a frase. A narradora onisciente tem autoridade máxima sobre o texto, mas pode perdê-la se parecer que está explicando demais. A narradora-personagem tem autoridade sobre sua própria experiência, mas não sobre o que não pode saber. A autoridade que a ficção estabelece define quanto a leitora aceita o que está sendo dito.",
    "conclusao":     "A conclusão na ficção não é o fim da história — é o ponto em que a narrativa faz sentido retroativamente. A boa conclusão não resolve tudo: resolve o suficiente para que a leitora sinta que o percurso valeu. A conclusão precipitada fecha o que deveria ficar aberto; a conclusão adiada frustra o que a narrativa prometeu. O final que a leitora lembra meses depois é o que a conclusão tornou possível sentir.",
    "aprendiz":      "O aprendiz na ficção é a personagem em formação — quem está aprendendo um ofício, uma arte, uma forma de viver, sob a tutela de outra personagem. O arco do aprendiz é o arco clássico: ignorância, erro, aprendizado, competência. Mas a ficção contemporânea complica esse arco — o aprendiz que supera o mestre, que descobre que o mestre estava errado, que aprende o contrário do que foi ensinado.",
    "apendice":      "O apêndice literário é o material que fica depois do fim da história — notas, documentos, mapas, genealogias, glossários. Na ficção, o apêndice pode ser recurso narrativo: o que está no apêndice existe no mundo da história, mas não na narrativa principal. O apêndice que a leitora consulta para entender o texto é parte da obra; o que ela ignora sem perda também é parte da obra — mas de forma diferente.",
    "engajamento":   "O engajamento na ficção é o estado em que a leitora não consegue parar — quando a narrativa criou a tensão certa para que cada pausa seja temporária. O engajamento é resultado de técnica: personagem com quem nos importamos, conflito com peso real, ritmo que avança sem apressar. A leitora engajada não lê para saber o que acontece: lê porque está dentro do mundo e não quer sair.",

    // Mercado editorial e processo criativo — v928
    "reescrita":     "A reescrita é a versão que a primeira versão tornava possível — o trabalho real depois do rascunho. Reescrever não é corrigir: é ver o texto de outro lugar, entender o que estava tentando ser e ajudar a chegar lá. A reescrita pode mudar a estrutura, o ponto de vista, a ordem, o tom. As escritoras que chamam de 'rascunho' o texto que publicam muitas vezes confundem coragem com ausência de processo.",
    "agente":        "O agente literário é o intermediário entre a escritora e o mercado editorial — quem representa o livro diante das editoras, negocia contratos, cuida dos direitos. No Brasil, a agência literária é mercado mais recente e mais concentrado que em outros países. Ter ou não ter agente muda o acesso: muitas editoras de grande porte recebem manuscritos só através de agentes.",
    "contrato":      "O contrato editorial é o documento que estabelece a relação entre a autora e a editora — o que a editora pode fazer com o texto, por quanto tempo, em que territórios, e quanto vai pagar. O contrato que a autora assina sem ler ou sem entender pode ceder direitos que ela precisará recuperar no futuro. Ler contratos, entender cláusulas e negociar condições é parte do ofício literário profissional.",
    "royalties":     "Os royalties são a porcentagem do preço de capa que cabe à autora por cada exemplar vendido. No Brasil, os percentuais de royalties variam em geral entre 10% e 15% do preço de capa, dependendo do contrato e da editora. O royalty é a principal forma de remuneração da autora ao longo do tempo — o que continua chegando depois do adiantamento. Entender o cálculo de royalties é entender quanto vale um livro para quem o escreveu.",
    "direitos":      "Os direitos autorais na ficção têm duas dimensões: os direitos morais (que pertencem à autora para sempre — o direito de ser reconhecida como criadora) e os direitos patrimoniais (que podem ser cedidos, licenciados, negociados). Os direitos de tradução, de adaptação, de audiovisual: cada um é negociável separadamente. Entender quais direitos está cedendo é entender o que está vendendo — e o que está guardando.",
    "cessao":        "A cessão de direitos é a transferência dos direitos patrimoniais de uma obra para outra pessoa ou empresa. A cessão pode ser total ou parcial, temporária ou permanente, para um território ou para o mundo. A cessão total e permanente é a forma mais drástica: a autora perde o controle comercial do próprio texto. O mercado editorial brasileiro usa frequentemente a cessão de direitos exclusivos por período determinado — o que é diferente de ceder para sempre.",
    "criatividade":  "A criatividade na ficção não é talento misterioso: é o resultado de ler muito, praticar muito, observar com atenção e arriscar combinações novas. A criatividade literária não vem do nada — vem da saturação: quem conhece profundamente uma tradição tem mais liberdade de se mover dentro e fora dela. O bloqueio criativo frequentemente é sinal de que a escritora precisa de mais entrada — mais leitura, mais experiência, mais tempo para a mente processar.",
    "originalidade": "A originalidade na ficção não é a ausência de influências — é uma voz que reconhecemos como própria, mesmo sabendo de onde veio. Toda obra origina de outras obras; a questão é o que foi feito com esse material. A originalidade que a crítica valoriza não é a que não deve nada a ninguém (impossível), mas a que transforma o que recebeu em algo que não existia antes.",
    "imitacao":      "A imitação na ficção é a prática deliberada de escrever como outra escritora — para aprender como ela faz. Imitar não é plagiar: é estudo. A escritora que imita Clarice aprende algo sobre sintaxe da emoção; quem imita Guimarães Rosa aprende algo sobre inventar língua. A imitação consciente é escola literária; a imitação inconsciente, sem digestão, é o que produz a voz que parece de outro.",
    "microficcao":   "A microficção é o texto muito breve — em geral menos de trezentas palavras — que conta uma história completa. A microficção exige precisão cirúrgica: cada palavra carrega mais peso porque há menos palavras. O texto de uma linha que contém um mundo inteiro de implicação — o famoso 'À venda: sapatos de bebê, nunca usados' — é o ideal da microficção: a história que está toda no que não foi dito.",
    "noveleta":      "A noveleta é a forma intermediária entre o conto e o romance — mais longa que o conto, mas sem a amplitude do romance. Não há consenso sobre os limites exatos, mas em geral é o que vai de dez mil a quarenta mil palavras. A noveleta permite o desenvolvimento de personagens e de trama que o conto não tem espaço para conter, sem exigir a arquitetura complexa que sustenta um romance.",
    "ciclo":         "O ciclo literário é uma série de textos com personagens, lugares ou temas recorrentes que formam um conjunto maior — sem ser necessariamente uma saga com ordem linear. As histórias do sertão de Guimarães Rosa formam um ciclo; os contos de um mesmo bairro podem ser um ciclo. O ciclo permite ao leitor aprofundar o conhecimento de um mundo a cada entrada, sem depender de ter lido os textos anteriores.",

    // Espaço, relações e tempo — v927
    "paisagem":      "A paisagem na ficção é o mundo visível em que a narrativa acontece — mas nunca é neutra. A paisagem que a narradora descreve é a paisagem filtrada pela personagem: o que ela nota, o que a afeta, o que ela ignora. A paisagem árida do sertão nordestino, a floresta amazônica, a favela vista de dentro e de fora — cada paisagem brasileira carrega sua história antes de qualquer personagem entrar.",
    "cenario":       "O cenário na ficção é o lugar imediato da cena — mais específico que paisagem, mais concreto que ambientação. O cenário é onde a cena acontece agora: a cozinha, a rua, o quarto. Um cenário bem construído não é descrição decorativa: é espaço que reage às personagens e que as personagens transformam ao habitar.",
    "ambientacao":   "A ambientação é o processo de criar o mundo da ficção — não apenas descrever onde se passa, mas fazer a leitora sentir que está lá. A ambientação bem-feita é discreta: não para a narrativa para explicar onde estamos, mas deixa o espaço vazar pela ação e pelo diálogo. A ambientação que precisa de parágrafos de descrição ainda não encontrou sua forma.",
    "lugar":         "O lugar na ficção é o espaço com identidade — não apenas onde algo acontece, mas um onde que importa. O lugar tem história, tem cheiro, tem o que ficou e o que foi embora. A ficção que cria um lugar, não apenas um cenário, cria algo que a leitora vai procurar novamente. Os grandes lugares da ficção brasileira — o sertão de Guimarães Rosa, o Rio de Lima Barreto — são personagens tanto quanto os humanos.",
    "aliado":        "O aliado na ficção é quem anda ao lado da protagonista — não necessariamente por concordar com ela, mas por ter interesse comum no mesmo caminho. O aliado que diverge da protagonista em método ou valores mas mantém o objetivo comum cria tensão interna rica. O aliado que se torna oponente, ou o oponente que se torna aliado, é um dos movimentos mais reveladores da narrativa.",
    "oponente":      "O oponente na ficção é quem se coloca no caminho da protagonista — não necessariamente por maldade, mas por ter objetivos conflitantes. Um oponente que acredita que está certo é mais interessante que um oponente que sabe que está errado. A ficção contemporânea tem se afastado do vilão cartunesco: o oponente mais perturbador é o que tem razões.",
    "rival":         "O rival na ficção é o oponente que espelha a protagonista — que quer a mesma coisa, usa as mesmas ferramentas, pode chegar ao mesmo lugar. O rival revela a protagonista por contraste: as mesmas ambições, mas escolhas diferentes. A rivalidade literária mais rica é a que faz a protagonista (e a leitora) perguntar: e se eu fizesse como ela faz?",
    "cumplice":      "O cúmplice na ficção é quem sabe o segredo — quem testemunhou o que não devia, quem participou do que não pode ser dito. A cumplicidade cria vínculo mas também risco: o cúmplice pode se tornar ameaça, testemunha, confidente ou traidor. A cumplicidade literária tem peso moral: que que os personagens decidiram fazer juntos, e o que isso fez com cada um deles?",
    "expansao":      "A expansão temporal na ficção é quando a narrativa desacelera — quando uma cena breve no tempo real ocupa muitas páginas. A batalha que durou uma hora descrita em cinquenta páginas; o momento de insight descrito em câmera lenta. A expansão é recurso de ênfase: o que a narrativa desacelera é o que ela declara importante. Usada em excesso, vira lentidão; usada com precisão, vira intensidade.",
    "simultaneidade": "A simultaneidade na ficção é quando duas coisas acontecem ao mesmo tempo em lugares diferentes — e a narrativa precisa decidir como mostrar isso. Uma técnica é alternar entre os fios; outra é narrar um e deixar o outro implicado. A simultaneidade cria tensão porque a leitora sabe o que uma personagem não sabe: o que está acontecendo ao mesmo tempo, em outro lugar.",
    "velocidade":    "A velocidade da narrativa é o quanto o tempo da história avança em relação ao tempo da narração. Uma narrativa rápida resume dias em parágrafos; uma lenta expande instantes em capítulos. A velocidade não é ritmo uniforme: o contraste entre passagens rápidas e lentas é que cria a sensação de movimento e de pausa. O que a narrativa desacelera é o que ela diz que importa.",
    "fluidez":       "A fluidez na prosa é a qualidade que faz a leitora esquecer que está lendo. A frase que flui não chama atenção para si mesma: transporta sem obstáculo. A fluidez não é facilidade — exige controle preciso de ritmo, conectivos, extensão de frase e vocabulário. A prosa que trava, que exige releitura constante, pode ser deliberada; a que trava por descuido é ruído.",

    // Trajetória, herança e luz — v926
    "trajetoria":    "A trajetória na ficção é o caminho percorrido — não o destino, mas o percurso que o constitui. A trajetória de uma personagem é revelada pelo que ela escolheu, recusou, perdeu e encontrou ao longo do tempo. Escrever a trajetória de um personagem é escrever as marcas que o tempo deixou nela — não como linha reta, mas como acumulação de desvios.",
    "percurso":      "O percurso na ficção é o caminho físico ou simbólico que a personagem faz — e que faz algo com ela. O percurso do romance de formação é aprendizado; o do romance policial é investigação; o da ficção de viagem é descoberta. O percurso que não muda quem o faz é deslocamento sem narrativa.",
    "caminho":       "O caminho na ficção é tanto literal quanto metafórico — a estrada que a personagem percorre e as escolhas que organizam sua vida. O caminho não percorrido (o que a personagem recusou, o que abandonou) é tão narrativo quanto o percorrido: define por negação o que a personagem é. A ficção de formação frequentemente estrutura o arco da personagem como um caminho com bifurcações.",
    "reencontro":    "O reencontro na ficção é o momento em que dois elementos que se separaram voltam a se tocar — pessoas, partes de uma identidade, uma personagem e seu passado. O reencontro raramente é o que foi prometido: o tempo mudou o que foi separado, e a expectativa e a realidade divergem. Essa divergência entre o reencontro esperado e o reencontro real é território narrativo rico.",
    "legado":        "O legado na ficção é o que fica depois — o que uma personagem ou geração deixa para a que vem. O legado pode ser herança concreta (dinheiro, terra, nome) ou intangível (trauma, orgulho, modo de ver). A personagem que carrega o legado de quem veio antes está carregando peso e possibilidade ao mesmo tempo. O legado que a ficção examina muitas vezes é o que ninguém pediu para carregar.",
    "tradicao":      "A tradição na ficção é o que foi transmitido e que organiza o presente — não como museu, mas como força viva que opera nas escolhas da personagem. A personagem que rompe com a tradição está se definindo em relação a ela; a que a abraça também. A tradição literária brasileira é múltipla e contraditória: oral e escrita, indígena e colonial, popular e erudita, e todas ao mesmo tempo.",
    "limite":        "O limite na ficção é onde algo acaba e outro começa — o fim do que é tolerável, do que é possível, do que é permitido. A personagem que chega ao limite muda: ou atravessa e vira outra pessoa, ou recua e aprende o que não estava disposta a fazer. O limite que a narrativa instala pode ser moral, emocional, físico ou social — e todos revelam a personagem de forma diferente.",
    "borda":         "A borda na ficção é o lugar de fronteira — onde o centro encontra o que está fora, onde o seguro termina e o desconhecido começa. A personagem que vive na borda não é nem de um lado nem de outro: essa posição pode ser marginalização ou pode ser perspectiva privilegiada — ver os dois lados porque não está em nenhum. A ficção de borda é a ficção que narra o que as bordas revelam.",
    "claridade":     "A claridade na ficção é luz que permite ver — e o que permite ver nem sempre é confortável. A claridade narrativa é o momento em que a névoa se dissipa e a personagem (e a leitora) vê o que estava oculto. A claridade pode ser resolução, pode ser revelação perturbadora ou pode ser a percepção de que o que parecia complexo é simples, e o que parecia simples é impossível.",
    "enredo":        "O enredo é a sequência de eventos organizados com causalidade — o que acontece e por quê. E. M. Forster distinguiu estória (o que acontece em ordem cronológica) de enredo (o que acontece com nexo causal): 'O rei morreu, depois a rainha morreu' é estória; 'O rei morreu, e depois a rainha morreu de tristeza' é enredo. O enredo cria necessidade onde a estória cria apenas sequência.",

    // Formas epistolares, duplo e publicação independente — v925
    "correspondencia": "A correspondência na ficção é a forma que sobreviveu ao tempo porque revela o que a fala presencial não consegue: o que se escreve quando há distância, quando há tempo para pensar, quando a carta vai carregar o que não cabia em conversa. A ficção epistolar usa a correspondência como estrutura: as cartas revelam as personagens pela forma como escrevem, não só pelo que dizem.",
    "diario":        "O diário na ficção é o texto que a personagem escreve para si mesma — sem leitor previsto, com a liberdade que isso permite. Por isso o diário literário tem uma voz diferente de qualquer outra forma: mais próxima, mais contraditória, mais disposta a não ter razão. Mas o diário que a leitora lê já é ficção dupla: ninguém escreve diário exatamente como pensa.",
    "carta":         "A carta na ficção é endereçada — tem um 'eu' e um 'você', e esse endereçamento muda tudo. O que se escreve para alguém específico é diferente do que se escreve no diário ou na narração. A carta também é documento de tempo: quem escreveu não sabe o que vai acontecer depois de mandar, não sabe como a carta vai chegar, não sabe se vai chegar.",
    "epistola":      "A epístola é a carta formal ou literária — a que tem consciência de ser texto, de poder ser lida além do destinatário imediato. As epístolas de Paulo de Tarso, as cartas de Kafka para Felice, as cartas de Clarice Lispector: a epístola literária move-se entre o privado e o público, entre a intimidade e a posteridade. Escrever carta sabendo que pode ser lida é escrever de forma diferente.",
    "duplo":         "O duplo na ficção é a figura que espelha a personagem principal — não como cópia, mas como versão alternativa, sombra, o que ela poderia ter sido ou o que ela teme se tornar. O duplo literário tem história longa: Poe, Dostoiévski, Stevenson, Wilde. Na ficção contemporânea, o duplo pode ser também estrutural: duas narrativas que se espelham, dois períodos de tempo que se respondem.",
    "sosia":         "O sósia na ficção é o duplo físico — quem se parece com a personagem de forma perturbadora. O sósia levanta questões sobre identidade: se alguém tem o mesmo rosto, tem a mesma pessoa? A confusão de sósias é recurso cômico clássico; o sósia que assume a vida de outro é recurso de thriller e de horror. O sósia desconfortável é o que faz a personagem questionar o que é essencial em si mesma.",
    "admiracao":     "A admiração na ficção é a emoção que eleva — o que a personagem sente por quem a excede em algo que ela valoriza. A admiração literária não é inveja: é reconhecimento. Mas pode ser ponto de virada: a personagem que admira pode querer se tornar, pode se apagar diante de, pode ser libertada por essa admiração quando entende que o admirado é falível como ela.",
    "fascinacao":    "O fascínio é a admiração que atrapalha — a atenção que a personagem não consegue desviar de algo ou alguém, mesmo quando seria prudente desviar. O fascínio na ficção tem perigo embutido: a personagem fascinada perde objetividade, perde perspectiva, pode tomar decisões que o fascínio distorce. O fascínio que dura muito tempo sem ser questionado torna-se obsessão.",
    "repulsa":       "A repulsa na ficção é a emoção que afasta — o que a personagem não consegue suportar, olhar, tocar. Mas a repulsa frequentemente revela tanto quanto a atração: o que uma personagem acha insuportável diz sobre seus valores, sua história, seus medos. A repulsa que a narrativa examina pode mostrar que o que repele a personagem é o que ela não quis reconhecer em si mesma.",
    "nojo":          "O nojo na ficção é a repulsa física — a reação do corpo diante do que é percebido como contaminante. Mas o nojo social — o nojo de classe, de raça, o nojo que organiza quem é considerado sujo ou inadequado — é tema narrativo muito mais rico e perturbador. A ficção que trabalha com nojo está trabalhando com as hierarquias que o corpo aprende a reproduzir antes que a mente perceba.",
    "fanzine":       "O fanzine é a publicação de fã e de entusiasta — feita fora da indústria, com recursos próprios, distribuída por amor a um tema ou comunidade. No Brasil, o fanzine literário tem história nos saraus, nas feiras independentes, nas comunidades de escritoras que publicam o que o mercado não publicaria. O fanzine é forma de existir antes de ser reconhecido pelo sistema.",
    "zine":          "O zine (pronunciado 'zain', de magazine) é a publicação artesanal de baixo custo e alta liberdade. No contexto literário brasileiro contemporâneo, o zine é espaço de experimentação formal e de vozes que a edição comercial marginaliza. Fazer um zine é publicar sem pedir permissão — e essa liberdade tem consequências estéticas e políticas.",

    // Religiosidade brasileira, trauma e biomas — v923
    "candomble":     "O candomblé na ficção brasileira é presença religiosa, cultural e política ao mesmo tempo — não folclore, não exotismo, mas sistema de conhecimento e de comunidade que sobreviveu à escravidão e ao apagamento colonial. A ficção que inclui o candomblé com seriedade não o usa como cor local: entende que é uma visão de mundo completa, com suas divindades, seus rituais, sua ética e sua relação com o tempo e a natureza.",
    "umbanda":       "A umbanda é religião de síntese brasileira — que incorpora elementos africanos, indígenas, do kardecismo e do catolicismo numa forma própria. Na ficção, a umbanda aparece frequentemente como espaço de cura, de encontro entre vivos e mortos, de acesso a forças que escapam à explicação racional. A ficção que trata a umbanda com respeito está reconhecendo que ela não é superstição: é tradição espiritual com profundidade própria.",
    "orixa":         "O orixá é a divindade do candomblé — força da natureza, energia ancestral, presença que governa aspectos da vida humana. Ogum, Oxum, Iemanjá, Xangô, Exu — cada orixá tem suas cores, seus ritmos, seus domínios. Na ficção afro-brasileira contemporânea, os orixás aparecem não como decoração: são forças narrativas que organizam o mundo das personagens de dentro, não de fora.",
    "sincretismo":   "O sincretismo religioso brasileiro é a mistura que a colonização forçou e que os povos escravizados e indígenas transformaram em resistência criativa. Associar orixás a santos católicos não foi capitulação — foi estratégia de sobrevivência que preservou a tradição. Na ficção, o sincretismo é campo de tensão e de criatividade: o Brasil que reza para Nossa Senhora Aparecida e para Iemanjá ao mesmo tempo é um Brasil que existe.",
    "fe":            "A fé na ficção não é sinônimo de religião institucionalizada — é a disposição de confiar em algo além do verificável. A personagem com fé tem uma âncora que a ficção pode explorar: o que sustenta quando tudo desmorona, o que a faz agir sem garantia. A ficção que respeita a fé de suas personagens sem julgá-la está tratando como real o que é real para elas.",
    "reza":          "A reza na ficção brasileira carrega memória de corpo — as palavras que a avó murmurava, o rosário que foi passado de mão em mão, o pedido feito num momento de desespero. A reza é linguagem do limite: quando o que pode ser feito já foi feito, a reza é o gesto que resta. Na ficção de comunidades populares brasileiras, a reza não é crendice: é prática de presença e de comunidade.",
    "ferida":        "A ferida na ficção é literalmente o dano — físico ou emocional — que a personagem carrega e que organiza como ela está no mundo. A ferida que não cicatriza é motor narrativo: a personagem age a partir dela, evita o que a reativaria, busca o que poderia curá-la. A ficção que respeita as feridas de suas personagens não as resolve com facilidade: sabe que cicatrizar leva tempo e que algumas feridas nunca fecham completamente.",
    "abuso":         "O abuso na ficção é poder usado para machucar — dentro de relações de confiança, de hierarquia, de intimidade. A ficção que narra abuso precisa de precisão e de posicionamento: mostrar sem espetacularizar, nomear sem explorar. O abuso que acontece dentro de família, de instituição, de relação amorosa tem estruturas que a ficção pode iluminar — mostrando como o abusado aprendeu a não ver o que acontecia, como o sistema falhou em protegê-lo.",
    "assedio":       "O assédio na ficção é a violência que opera na acumulação — o que cada episódio isolado parece pequeno, mas juntos constroem um ambiente de opressão. A ficção que narra assédio enfrenta o mesmo problema que a vítima: a dificuldade de fazer outros entenderem o que cada incidente individual parecia tolerar. O assédio na narrativa contemporânea brasileira está sendo nomeado com a precisão que a literatura precisou ganhar para nomear o que antes ficava em silêncio.",
    "amazonia":      "A Amazônia na ficção brasileira é mais do que cenário — é personagem. A floresta que respira, que tem memória, que está sendo destruída em câmera lenta: qualquer ficção ambientada na Amazônia carrega o peso dessa realidade. A ficção amazônica de autoria amazônica — ribeirinhos, indígenas, comunidades da floresta narrando seu próprio território — é diferente da ficção sobre a Amazônia feita de Brasília ou São Paulo.",
    "floresta":      "A floresta na ficção é espaço de abundância e de ameaça ao mesmo tempo — onde a vida pulsa com intensidade que a cidade não tem, e onde o ser humano não é o centro. Na ficção brasileira, a floresta é frequentemente a Amazônia, mas também a Mata Atlântica devastada, o Cerrado queimado, o Pantanal alagado. Escrever floresta é escrever um ecossistema com presença própria, não um cenário vazio aguardando enredo.",
    "giria":         "A gíria na ficção é a linguagem que vive fora da norma — que nasce nas margens, que circula entre grupos específicos, que marca pertencimento. A gíria que a ficção usa tem vida própria: envelhece, migra, é apropriada. A escritora que usa gíria com consistência e respeito à sua origem está construindo mundo; a que a usa como fantasia de autenticidade sem conhecimento está construindo clichê.",

    // Identidades brasileiras e ofício de edição — v922
    "indigena":      "O indígena na ficção brasileira esteve por muito tempo como objeto de narrativa — o selvagem, o bom selvagem, o símbolo da natureza. A virada contemporânea é a voz indígena narrando a si mesma: escritoras como Eliane Potiguara, Graça Graúna e Daniel Munduruku e muitas mais que estão construindo uma literatura indígena em língua portuguesa e em línguas originárias. Essa literatura não é exótica: é o registro de povos com história, filosofia e literatura próprias que a colonização tentou silenciar.",
    "quilombola":    "A quilombola é a pessoa de comunidade quilombola — descendente dos quilombos, herdeira de uma forma de resistência e organização que o Brasil oficial tardou a reconhecer. A ficção de autoria quilombola está narrando territórios, memórias e identidades que a literatura hegemônica ignorou. A quilombola na ficção não é só símbolo de resistência passada: é presença viva que negocia com o Estado, que disputa terra, que cria cultura.",
    "ribeirinho":    "O ribeirinho é quem vive às margens dos rios — especialmente na Amazônia, onde os rios são estradas e os ciclos da água organizam a vida. A ficção ribeirinha traz uma relação com o tempo e o espaço radicalmente diferente da ficção urbana: a enchente que isola, a fartura da pesca, a floresta como vizinha permanente. A literatura amazônica está construindo esse vocabulário de pertença que a ficção sul-sudeste raramente alcançou.",
    "nordestino":    "O nordestino na ficção brasileira carrega o peso de décadas de representação paternalista — o retirante, a vítima da seca, o migrante anônimo. A ficção nordestina de autoria nordestina está recusando esse enquadramento: nordestinos como sujeitos de cultura rica, de humor preciso, de resistência cotidiana. Da literatura de cordel ao romance urbano de Recife ou Fortaleza, há uma produção que contradiz o clichê da miséria como única história.",
    "mineiro":       "O mineiro na ficção brasileira tem sua própria mitologia — reticente, reservado, de fala cautelosa e humor seco. Mas Minas Gerais é também território de contradições ricas: o barroquismo que persiste, as cidades históricas que habitam o presente, a tradição literária que vai de Drummond a Autran Dourado a Conceição Evaristo. O 'jeito mineiro' que a ficção às vezes caricatura é também um modo de ser que a literatura soube transformar em voz.",
    "gaucho":        "O gaúcho na ficção tem uma tradição de narrativa épica e regionalista — o pampa, o cavalo, o chimarrão, a fronteira. Mas a ficção gaúcha contemporânea está expandindo esse arquivo: escritoras porto-alegrenses narrando a cidade, comunidades alemãs e italianas do interior narrando sua mistura, vozes que não cabem na iconografia do gaúcho tradicional. O sul do Brasil tem literaturas no plural.",
    "reescrever":    "Reescrever é o trabalho que transforma texto em literatura. A primeira versão existe para que a reescrita possa começar. Reescrever não é consertar: é ver o texto como um todo e perguntar o que está impedindo que ele seja o que quer ser. A escritora que não reescreve está entregando a primeira tentativa; a que reescreve pode entregar o que a primeira tentativa prometia.",
    "cortar":        "Cortar é um dos atos mais difíceis e mais transformadores da escrita. Cortar é reconhecer que um trecho não estava servindo ao texto — mesmo que seja bom isoladamente, mesmo que tenha custado tempo e esforço. O texto que não foi cortado onde deveria parece mais pesado do que é. A escritora que aprende a cortar sem pena está aprendendo a servir o texto, não o ego.",
    "improvisar":    "Improvisar na escrita é escrever sem saber para onde vai — deixar a voz ir sem plano, sem estrutura prévia, no contato direto com o que emerge. A improvisação não é método de todos, mas é exercício valioso: revela o que a escritora pensa antes de editar o que pensa. O que surge no improviso frequentemente tem energia que a escrita planejada não encontra.",
    "declamar":      "Declamar é ler o texto em voz alta com presença — não só para verificar o ritmo, mas para habitá-lo. A slam poetry e a poesia performática brasileira vivem na declamação: o texto é inseparável do corpo que o diz. Mas qualquer texto se beneficia de ser lido em voz alta: o ouvido capta o que o olho passa. O parágrafo que trava na língua precisa ser reescrito.",
    "protagonismo":  "O protagonismo não é só ser personagem principal — é ter agência dentro da história, ser quem toma decisões que movem a narrativa. O protagonismo negado a personagens por razão de gênero, raça ou classe é violência narrativa: elas existem na história mas são movidas por ela, não a movem. A ficção que devolve protagonismo a quem historicamente não o teve está fazendo escolha política e estética ao mesmo tempo.",
    "vulnerabilidade": "A vulnerabilidade na ficção é o que torna a personagem humana — o ponto em que ela pode ser atingida, onde a armadura cede, onde o que é mais essencial fica exposto. A personagem sem vulnerabilidade não tem interior que o leitor possa alcançar. A vulnerabilidade bem escrita não é fraqueza: é a abertura que permite a conexão entre personagem e leitora.",

    // Espaços urbanos, tempo vivido e relações — v921
    "cemiterio":     "O cemitério na ficção é o espaço onde o presente e o passado se tocam de forma mais visível — onde a memória tem nome e data, onde a morte é organizada socialmente. O cemitério como cenário literário convida ao diálogo com quem não está mais — e à reflexão sobre quem vai continuar. Na ficção brasileira, o cemitério também é espaço de desigualdade: sepulturas de famílias e valas comuns não são iguais.",
    "porto":         "O porto na ficção é a fronteira da chegada e da partida — o lugar onde o mundo externo entra e onde a vida local se expõe ao que é de fora. O porto carrega história: na ficção brasileira, é onde chegaram escravizados, imigrantes, mercadorias; onde partiram exilados, emigrantes. O porto que a ficção contemporânea revisita frequentemente está revisitando essa história de atravessamentos forçados e escolhidos.",
    "estacao":       "A estação — de trem, de metrô, de ônibus — é na ficção o espaço do trânsito e do encontro acidental. É onde pessoas que nunca se encontrariam de outra forma se cruzam por instantes. A estação ferroviária brasileira que foi desativada é também espaço de nostalgia e de abandono: o que a modernidade prometeu e não cumpriu. A estação como cenário acumula chegadas e partidas que ela mesma esqueceu.",
    "bairro":        "O bairro na ficção é a escala humana da cidade — menor que a metrópole, maior que a rua. O bairro tem memória própria: o bar que fechou, a casa que foi demolida, a família que morava na esquina. A personagem que pertence a um bairro tem referências espaciais que a definem; a que perdeu o bairro perdeu uma camada de identidade. A ficção de bairro — presente na crônica brasileira — é a ficção do cotidiano com nome e endereço.",
    "periferia":     "A periferia na ficção brasileira é território político antes de ser geográfico. Periférico não é só quem mora longe do centro: é quem está à margem do que a cidade considera importante, legítimo, digno de narração. A literatura periférica brasileira — de Ferréz a Sacolinha, de Conceição Evaristo a vozes mais jovens — não está pedindo autorização para narrar: está narrando o que sempre existiu e nunca foi considerado literatura.",
    "urgencia":      "A urgência na ficção é o que comprime o tempo — a sensação de que algo precisa acontecer antes que seja tarde demais. A urgência acelera o texto: frases mais curtas, cortes mais bruscos, menos digressão. A escritora que domina a urgência sabe quando e como instalá-la — e sabe que não pode sustentar urgência constante ou o leitor anestesia. A urgência precisa de contraste com o descanso.",
    "espera":        "A espera na ficção é tempo que não passa — e que por não passar, revela. O que a personagem pensa enquanto espera, o que percebe que normalmente não veria, o que deseja e teme: a espera é a pausa forçada em que o interior da personagem emerge. A ficção que sabe construir espera tem paciência; a que a evita está com pressa de chegar ao evento e perdendo o que acontece no intervalo.",
    "demora":        "A demora na ficção é a lentidão narrativa intencional — o que acontece quando a história diminui a velocidade para que algo seja percebido com mais cuidado. A demora que funciona não é prolixidade: é atenção ampliada. O parágrafo que descreve um gesto em detalhe onde caberia uma linha está apostando que aquele gesto importa mais do que parece. A demora precisa pagar essa aposta.",
    "paciencia":     "A paciência na ficção é virtude e estratégia ao mesmo tempo — a personagem que espera porque sabe que agir agora custaria mais do que esperar está exibindo uma forma de inteligência. Mas a paciência pode também ser resignação disfarçada, passividade racionalizada. A ficção que questiona a paciência está perguntando: o que você realmente está esperando? E até quando?",
    "antecipacao":   "A antecipação é o que o leitor sente quando sabe que algo está para acontecer — quando as pistas estão postas, a tensão está instalada, e o evento está iminente. Criar antecipação é criar desejo no leitor de continuar lendo. A antecipação que não se cumpre em algo à altura frustra; a que se cumpre além do esperado surpreende. O controle da antecipação é um dos poderes centrais da narrativa.",
    "companheirismo": "O companheirismo na ficção é a versão mais discreta da amizade — não a intensidade da eleição mútua, mas a constância da presença ao lado. A personagem que tem companheiro de ofício, de percurso, de resistência tem um recurso narrativo valioso: alguém com quem pensar em voz alta, com quem dividir o peso sem precisar ser herói solitário. O companheirismo tem a textura do cotidiano, não do drama.",
    "retornar":      "Retornar na ficção é um dos movimentos mais carregados de sentido: voltar para um lugar ou uma relação depois de uma ausência transforma tanto o que voltou quanto o que ficou. O retorno na ficção clássica é a chegada do herói; na ficção contemporânea, é frequentemente decepção — o lugar mudou, ou a personagem mudou, ou as duas coisas, e o reencontro ilumina exatamente essa distância.",

    // Psicologia profunda, cultura oral e espaços culturais — v920
    "ego":           "O ego na ficção não é o conceito psicanalítico técnico — é a camada da personagem que ela apresenta ao mundo, o que ela acredita ser e o que defende como identidade. O ego que a narrativa coloca em crise está colocando a personagem diante do que não quis ver sobre si mesma. A ficção psicológica opera frequentemente pela erosão do ego até chegar ao que estava embaixo.",
    "inconsciente":  "O inconsciente na ficção aparece nos sonhos, nos lapsos, nos desejos que a personagem não admite conscientemente. A ficção que trabalha com inconsciente não precisa nomear Freud: precisa mostrar o que a personagem faz sem entender por quê, o que atrai sem que ela saiba, o que repete sem perceber que repete. O inconsciente literário é o que a narração sabe que a personagem não sabe.",
    "arquetipo":     "O arquétipo é a figura universal que aparece em culturas diferentes com formas semelhantes — a Mãe, o Herói, o Trickster, a Sombra. Jung identificou arquétipos como estruturas do inconsciente coletivo. Na ficção, os arquétipos são convocados conscientemente ou não: a personagem que assume um arquétipo ressoa em leitores de culturas distintas porque toca algo comum. Mas o arquétipo sem especificidade cultural vira clichê.",
    "folclore":      "O folclore é o saber e o narrar que circula entre pessoas comuns — contos, provérbios, crenças, receitas, rezas, lendas — sem autora conhecida, passado de boca em boca. O folclore brasileiro é múltiplo e contraditório: carrega memória indígena, africana e europeia misturadas de formas que nenhum dessas tradições isoladas conseguiria prever. A escritora que dialoga com o folclore está entrando numa tradição muito mais vasta do que a da literatura escrita.",
    "lenda":         "A lenda é a história que circula como se pudesse ter acontecido — diferente do mito (que é sagrado e fundador) e do conto (que sabe que é ficção). A lenda brasileira tem território: a Iara vive nos rios, o boto encantado seduz na Amazônia, o Saci vive no mato. As lendas marcam o espaço com presença sobrenatural e explicam o que a ciência não alcança — ou o que a comunidade prefere explicar de outro modo.",
    "bestiario":     "O bestiário é o catálogo literário de criaturas — reais, fantásticas ou hibridizadas. Na Idade Média europeia, o bestiário era tratado enciclopédico e moral ao mesmo tempo: cada animal tinha um significado simbólico. Na literatura brasileira contemporânea, o bestiário reaparece como forma de nomear criaturas do imaginário local — com ironia, com reverência ou com ambos.",
    "metrica":       "A métrica é o sistema de contagem de sílabas que organiza o verso formal. O verso de dez sílabas (decassílabo), o de sete (redondilha maior), o de cinco (redondilha menor) — cada um tem cadência própria e história longa na tradição lusófona. A poesia moderna frequentemente abandona a métrica fixa; mas entendê-la permite saber o que se está abandonando e por quê.",
    "enjambamento":  "O enjambamento é quando o sentido da frase no verso não termina com o verso — continua para o seguinte, quebrando a expectativa de pausa. O enjambamento cria tensão entre a unidade do verso e a unidade da frase: o leitor é puxado adiante contra a quebra de linha. É um dos recursos de ritmo mais poderosos da poesia porque opera no corpo — na hesitação, na aceleração, no suspiro.",
    "haicai":        "O haicai é a forma poética japonesa de três versos (5-7-5 sílabas) que busca capturar um instante — geralmente natural, sensorial, transitório. No Brasil, o haicai chegou com os imigrantes japoneses no século XX e criou uma linhagem própria, com escritoras que adaptaram a forma ao clima, ao vocabulário e à sensibilidade brasileiras. O haicai é um exercício de presença e de seleção radical do essencial.",
    "biblioteca":    "A biblioteca na ficção é o espaço do saber acumulado — mas também do saber guardado, às vezes inacessível, às vezes perigoso. A biblioteca como cenário literário tem tradição longa: de Babel a Alexandria, de O Nome da Rosa ao arquivo de Hogwarts. Na ficção brasileira, a biblioteca que não existe — o acesso ao livro que foi negado, o analfabetismo como exclusão — é também narrativa.",
    "arquivo":       "O arquivo na ficção é o lugar onde o passado está guardado — mas nunca está completo. O que o arquivo não tem diz tanto quanto o que tem. A personagem que vasculha um arquivo está procurando o que foi deliberadamente omitido ou simplesmente perdido. Na ficção brasileira, o arquivo é frequentemente espaço político: de que história se guarda prova, e de quem se apaga o rastro.",
    "teatro":        "O teatro como espaço na ficção é onde o real e o representado coexistem de forma mais visível — o palco onde todos sabem que é ficção e mesmo assim creem. A personagem que vai ao teatro está entrando num espaço de convenção consentida. O teatro como metáfora (a vida como palco, o mundo como teatro) é um dos mais antigos da literatura ocidental — e um dos mais gastáveis se não for renovado.",

    // Espaço doméstico, corpo e léxico — v919
    "vocabulario":   "O vocabulário de uma escritora é o conjunto de palavras que ela tem disponível — e que escolhe usar. Vocabulário amplo não é ostentação: é precisão. A palavra certa para uma coisa específica existe, e encontrá-la é parte do ofício. O vocabulário literário não é o vocabulário do dicionário: é o que a escritora construiu pela leitura, pela atenção ao mundo e pela prática de escrever.",
    "sinonimo":      "Os sinônimos raramente são idênticos: cada palavra carrega tom, origem e frequência diferentes. 'Falecer' não é igual a 'morrer', que não é igual a 'partir'. A escritora que trata sinônimos como intercambiáveis perde precisão; a que escolhe entre eles com critério — pelo som, pelo registro, pelo que a palavra evoca — afina o texto.",
    "antonimo":      "Os antônimos são palavras opostas em significado — mas na ficção o interesse não está nos opostos absolutos, e sim nos graus entre eles. O contrário de 'amor' não é só 'ódio': pode ser 'indiferença', 'descuido', 'ausência'. A ficção que trabalha com gradações entre opostos tem mais textura do que a que opera em binários.",
    "horizonte":     "O horizonte na ficção é o que a personagem vê como possível — o limite do que imagina para si mesma. Uma personagem sem horizonte não projeta futuro; uma com horizonte amplo enxerga possibilidades que outros não veem. O horizonte também é espaço literal: a linha entre terra e céu que organiza o olhar, que diz onde termina o que é conhecido.",
    "gesto":         "O gesto na ficção revela o que a fala esconde — ou confirma o que a fala não precisaria dizer. A personagem que fala de calma enquanto os dedos tamborilem na mesa não está calma. O gesto bem escolhido substitui parágrafos de explicação psicológica: 'ela desviou o olhar' diz mais sobre o que sentiu do que 'ela sentiu vergonha'.",
    "postura":       "A postura da personagem — como ela ocupa o espaço físico, como segura o corpo diante de outras pessoas — é linguagem não-verbal que a ficção pode usar. A personagem que se encolhe em determinadas presenças e se expande em outras está revelando sua economia de poder. Escrever postura é escrever relação.",
    "respiracao":    "A respiração na ficção é ritmo e emoção ao mesmo tempo. Personagens que não conseguem respirar direito — por medo, por esforço, por choque — comunicam estado interno de forma física. O texto que presta atenção à respiração das personagens está prestando atenção ao corpo como registro emocional, não só à mente.",
    "varanda":       "A varanda na ficção brasileira é espaço de transição — entre dentro e fora, entre privado e público, entre o que se é e o que se mostra. A varanda onde a família se reúne, onde se observa a rua, onde se recebe visita informal: cada uma dessas funções carrega um tipo de cena diferente. A varanda vazia diz uma coisa; a cheia, outra.",
    "quintal":       "O quintal na ficção brasileira é espaço de infância, de memória, de trabalho doméstico e de resistência. O quintal com plantas, com galinhas, com cheiro de terra depois da chuva — é cenário de uma vida que a ficção de elite frequentemente ignorou. O quintal não é fundo de cena: é personagem do cotidiano que a ficção periférica e regional soube nomear.",
    "corredor":      "O corredor na ficção é o espaço de passagem — mas na passagem, muito acontece. Conversas interrompidas, encontros não planejados, escutas acidentais: o corredor é onde o que não deveria ser dito frequentemente é dito. Em casas grandes de ficção brasileira (colonial, rural), o corredor organiza hierarquias; em apartamentos, revela proximidade forçada.",
    "juventude":     "A juventude na ficção é o tempo da possibilidade antes da escolha — quando tudo ainda poderia ser. A ficção de juventude tem urgência própria: a identidade ainda está sendo formada, as opções ainda parecem ilimitadas, o erro ainda pode ser corrigido. A ficção brasileira contemporânea de jovens está revisitando o que significa ser jovem em diferentes classes, regiões e identidades.",
    "maturidade":    "A maturidade na ficção é o estado de quem já escolheu — e vive com as consequências das escolhas. A personagem madura carrega história; as perdas estão marcadas nela. Mas a maturidade na ficção também pode ser questionada: o que chamamos de maturidade pode ser acomodação, pode ser resignação. A personagem que rompe com a maturidade esperada tem um arco.",
  ...
  function inferFuncaoSintatica(word, className, sentence) {
    ...
    var idx      = tokens.findIndex(function(t) { return (normalizeWord(t) === norm); });
    if (idx === -1) return "";
    ...
    var _GRAM_PFX = ["Substantivo","Verbo","Adjetivo","Advérbio","Conjunção","Preposição","Pronome","Artigo","Interjeição","Numeral"];
    var effectiveClass = _GRAM_PFX.some(function(p) { return (className.startsWith(p)); })
      ? className
    ...
    if (effectiveClass.startsWith("Adjetivo") || effectiveClass === "Verbo (particípio)") {
      var prevNorm = idx > 0 ? normalizeWord(tokens[idx-1]) : null;
      var _COP = new LSet(["estava","estou","esta","era","e","sou","sao","foi","fica","ficou","parecia","parece"]);
    ...
    var _PERF_SFX = /^.{3,}(ou|eu|iu|aram|eram|iram|ando|endo|indo|asse|esse|isse)$/;

    var verbIdx = -1;
    for (var i = 0; i < tokens.length; i++) {
      if (i === idx) continue;
      var n = normalizeWord(tokens[i]);
      ...
      var prevN = normalizeWord(tokens[i-1]);
      var nextN = i < tokens.length-1 ? normalizeWord(tokens[i+1]) : null;
      var tc = inferWordClassContextual(tokens[i], sentence, prevN, nextN);
    ...
    var PURE_PREPS = new LSet(["de","em","para","por","com","sem","sobre","ao","da","do","na","no","pelo","pela","nos","nas","aos","ate","apos","perante","ante","entre","desde","durante","mediante","conforme"]);
    var prevNorm = idx > 0 ? normalizeWord(tokens[idx-1]) : null;
    var prevPrev = idx > 1 ? normalizeWord(tokens[idx-2]) : null;
    // inPrepPhrase: prev é preposição, OU prev é artigo e prev2 é preposição ("para a cidade")
    var _ARTS_PT = new LSet(["a","o","os","as","um","uma"]);
    var inPrepPhrase = (prevNorm && PURE_PREPS.has(prevNorm))
    ...
    if (effectiveClass.startsWith("Substantivo") || effectiveClass.includes("Pronome pessoal") ||
      ...
      if (idx > verbIdx) {
        ...
        var _GRAM_SYN = new LSet(["a","o","os","as","um","uma","de","em","para","por","com","sem",
        ...
        var contentBeforeVerb = tokens.slice(0, verbIdx).filter(function(t) { return (!_GRAM_SYN.has(normalizeWord(t))); }).length;
  ...
  function createLocalNote(className, normalized) {
    ...
    if (className.includes("Substantivo") || className.includes("Substantivo próprio"))
        return "Substantivo concreto que ancora a cena pelo que o personagem faz com ele ou pelo que ele guarda. Se figurar só como cenário, avalie se o objeto pode carregar desejo, memória ou poder — assim vira motor, não decoração.";
    return "Palavra fora do dicionário curado — classificação por heurística; confira o sentido no contexto.";
  }
...
for (var __i_3 = 0; __i_3 < __iter_3.length; __i_3++) {
  var token = __iter_3[__i_3]; 
    ...
    var start = Math.max(0, matchIndex - 180);
    var end   = Math.min(cleanText.length, matchIndex + 240);
    var excerpt = cleanText.slice(start, end);
    var rel = matchIndex - start;
  ...
  function countWordOccurrences(text, word) {
    if (!text) return 0;
    return tokenizeWords(text).filter(function(t) { return (normalizeWord(t) === word); }).length;
  }
  ...
  global.VeredaLexical = {
    analyze: analyze,
    createHighlightedContext: createHighlightedContext,
    normalizeWord: normalizeWord,
    inferWordClass: inferWordClass,
    inferWordClassContextual: inferWordClassContextual,
    tokenizeWords: tokenizeWords,
    ensureLoaded: ensureLoaded,
    isLoaded: function() { return (_loaded); },
    hasLoadError: function() { return (_loadError); },
  };
})(typeof window !== 'undefined' ? window : global);
/* lexical.footer.js — registra o VeredaLexical (port ES5) no contrato Encore.
 * Apensa ao fim do engine gerado: define VeredaLexicalEngine (id/domain/version)
 * com check(snapshot, done) assíncrono → Findings (Encore.contracts.Finding).
 */
(function (G) {
  function VeredaLexicalEngine() {
    var self = this;
    this.id = "LEXICO-CLASSES";
    this.domain = "lexico-classes";
    this.version = "1.0.0";
    this.isLoaded = function () {
      return !!(G.VeredaLexical && G.VeredaLexical.isLoaded());
    };
    this.check = function (snapshot, done) {
      setTimeout(function () {
        var findings = [];
        try {
  ...
  G.VeredaLexicalEngine = VeredaLexicalEngine;
  if (typeof module !== "undefined" && module.exports) {
    module.exports = VeredaLexicalEngine;
  } else {
    G.Encore.core = G.Encore.core || {};
    G.Encore.core.engines = G.Encore.core.engines || {};
    G.Encore.core.engines.VeredaLexicalEngine = VeredaLexicalEngine;
  }
