...
(function (root) {
  ...
  function starts(s,w) { var i=s.indexOf(w),before,after;
    while(i!==-1){before=i===0?' ':s.charAt(i-1);after=s.charAt(i+w.length);
      if(!/[A-Za-zÀ-ÖØ-öø-ÿ]/.test(before)&&!/[A-Za-zÀ-ÖØ-öø-ÿ]/.test(after)){return true;}
      i=s.indexOf(w,i+1);
    }return false;
  }
  function indexed(s,limit) {
    var check=s.slice(0,limit), clean=E.protectedText?E.protectedText(check):check;
    var ts=E.reading.tokens(clean), keys={}, signals={}, i,t,w,parts,rule,key,prior,abbr;
    ...
    var expressions=E.styleData&&E.styleData.entries||[],low=clean.toLowerCase();
    for(i=0;i<expressions.length&&!signals.expressoes;i+=1){
      phrase=expressions[i].term;if(phrase&&starts(low,String(phrase).toLowerCase())){signals.expressoes=true;}
    }
    parts=clean.split(/\r\n|\n|\r/);
  ...
  function refresh(force) {
    ...
    for(var i=0;i<chosen.length;i++){id=chosen[i];
      ...
      btn.setAttribute('aria-label','Sinal encontrado para '+labels[id]);}
  ...
  function run() {
    ...
    for(k in labels){if(own.call(labels,k)&&draft.signals[k]){candidates.push(k);}}
  ...
  action.addEventListener('click',run,false);
