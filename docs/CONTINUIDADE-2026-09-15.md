...
Base inicial: PR 174, `eb8f381`, v6.1. Quadro publicado pelo PR 175, `f3ce417`, v6.2. O usuário autorizou executar e publicar. Não retomar a tarefa antiga abandonada e concluída pelo OpenCode.

## Ordem de entrega

1. Quadro de giz por caderno: publicado; testes Chromium/WebKit e capturas aprovados. Canvas 2D ES5, traços em `book.data.chalk`, pacotes existentes incluem o desenho, exportação própria `.giz.txt`, validação de tamanho, falha de gravação explícita e revisão contra sobrescrita por outra aba. Referência visual do AI Studio: papel sálvia/carvão, filetes, marcas de registro, calha, giz e apagador feitos por CSS estático.
...
PR 176 reúne a implementação dos itens 2 a 5, em validação. As decisões e os limites estão em `ORIENTACAO-LINGUISTICA-LINHAGEM-V6-3.md`. O cache incremental foi comparado ao motor integral: reuso das lentes lexicais sem alterar apontamentos; as lentes globais continuam integrais.
