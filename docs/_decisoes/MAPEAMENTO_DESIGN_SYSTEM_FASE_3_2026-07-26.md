...
A leitura estática foi feita sobre a `main` originalmente no commit `8176130650bd3c55639e72605ac2ca3ac862e36a`. O relatório foi atualizado após o squash merge do PR #134, no commit `29b75b3c6aff59ac1f207c65476293395b42065b`, e considera a seguinte sequência de baseline:

- Tarefa 1: o PR #134 foi incorporado; Vereda usa `--muted: #b7a996` e o auditor automático de contraste está ativo;
- Tarefa 2: o PR #135 foi reconstruído sobre a nova `main`, com cache `v961`, e permanece em rascunho aguardando autorização de merge; `cerrado-dark`, `mata-dark` e `amazonia-dark` são tratados como remoção aprovada e não aparecem como dívida desta fase.

O PR #134 está incorporado. O PR #135 continua em rascunho e não deve ser incorporado sem autorização explícita. Toda branch posterior deve partir da `main` atualizada e preservar versionamento de cache estritamente crescente.
...
Decisão aprovada para a correção de cor sintática:

As sete categorias gramaticais manterão tokens semânticos próprios, distintos das quatro cores de aviso. A diferenciação será reforçada por texto, rótulo e borda, de modo que o significado não dependa apenas da percepção cromática.

A implementação deve preservar a correspondência entre categoria, nome textual e tratamento de borda em Alvorada e Vereda. Não se deve reduzir automaticamente as sete categorias a `accent-primary` nem reutilizar os quatro papéis de aviso.
...
| Painel linguístico | paleta categórica e terceira coluna | `css/03-inspector-precision.css` | tokens sintáticos próprios aprovados |
...
12. Prática;
13. Autoria;
14. painel linguístico, aplicando os sete tokens sintáticos próprios aprovados.
...
6. migração das cores sintáticas para tokens próprios sem reduzir a diferenciação entre categorias.
