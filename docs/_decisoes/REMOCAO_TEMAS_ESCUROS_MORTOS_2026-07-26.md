...
- `ASSET_VERSION`: `20260726-lexical-hint-v1` → `20260726-remove-dead-dark-themes-v1`;
- `CACHE_NAME`: `vereda-offline-v959` → `vereda-offline-v960`;
- referências versionadas alinhadas em `index.html`, `service-worker.js` e `ui-dialog.js`.

Não houve mudança na lista de assets, estratégia, escopo, instalação, ativação, busca ou descarte de caches do service worker.

Como esta pull request e a Tarefa 1 nasceram separadamente da mesma `main`, ambas avançam inicialmente a partir de `v959`. Antes de qualquer incorporação sequencial, a segunda branch deverá ser atualizada sobre a `main` já incorporada e o número de cache deverá permanecer estritamente crescente.
